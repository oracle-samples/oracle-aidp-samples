...
| `dbutils.secrets.*` | âœ… | Backed by OCI Vault (API key auth). Requires `AIDP_VAULT_OCID` env var + secret named `<scope>/<key>`. |
| `dbutils.notebook.*` | âœ… | `notebook.run` and `notebook.exit` go to `oidlUtils.notebook.*`. Use `oidlUtils.notebook.run(path, timeout=3600)` for new code (timeout=0 is rejected by AIDP). |
