...
with OpenAI model (adaptive thinking + tools). The plans are then followed
...
DATABRICKS_PATTERNS = [
    ...
    # dbutils widget/job/notebook replacements — flagged here so OpenAI model sees them in changes_needed
...
RISK_PATTERNS = [
    ...
    },
    {
        ...
        # broader pattern so OpenAI model is prompted to check any TF model I/O.
    ...
    },
    {
        ...
        # Python-only. OpenAI model should port the Scala logic to Python (UDFs, DataFrame ops)
...
def analyze_cell_static(cell_index: int, cell_source: str,
    ...
    Returns a partial plan that can be enriched by OpenAI model.
    """
    plan = {
...
def render_cell_plan(plan: dict) -> str:
    """Render a cell plan as text for the OpenAI model migration prompt."""
