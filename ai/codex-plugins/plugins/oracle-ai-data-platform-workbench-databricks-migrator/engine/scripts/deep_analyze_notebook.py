#!/usr/bin/env python3
"""
Deep Notebook Analyzer - Uses OpenAI GPT-5.5 (1M context) via the OpenAI API
to do a thorough, line-by-line analysis of each notebook for AIDP migration.

This is NOT a regex scan. Each notebook gets a full OpenAI model analysis that:
...
def analyze_single_notebook(notebook_path: str, relative_path: str, report_dir: str) -> dict:
    """Analyze a single notebook using OpenAI GPT-5.5."""
...
def analyze_batch(notebook_list_path: str, concurrency: int = 3, start_index: int = 0, end_index: int = None):
    ...
    print(f"Model: OpenAI GPT-5.5 (1M context)")
...
def main():
    parser = argparse.ArgumentParser(description="Deep notebook analysis with OpenAI GPT-5.5")
