...
- OpenAI GPT-5.5 for analysis, migration, and fixing
...
from openai_model import OpenAIModelCompatClient, OPENAI_MODEL
import ast
import asyncio
import json
import os
...
# â”€â”€â”€ Output compaction for LLM prompts â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _compact_output_for_llm(text: str, max_chars: int = 6000) -> str:
    ...
    of times during long jobs â€” useless for LLM evaluation but blow up token count.
    """
    ...
    if repeat_count > 0:
        result.append(f"  ... [{repeat_count} similar lines collapsed]")
    compacted = "\n".join(result)
    # Final safety cap â€” keep first half + last half
...
# Generic tool â€” no hardcoded tenant/AIDP-instance config. These are provided
...
def build_oidlutils_bridge_snippet(output_base: str, job_name: str) -> str:
    ...
    and job_migrate_from_workflow.py (startup connect) â€” both call this so the
    snippet stays in sync.

    The wrapper reads getParameter from two files (lookup order):
      1. task_values.json    â€” written by setTaskValue (cross-task, mutable)
      2. manifest_params.json â€” written by process_notebook at task start
...
class _AIDPParams:
    ...
    def getTaskValue(self, taskKey, name, default=""):
        ...
        # by source task â€” names are unique per-job in practice).
        tv = self._read(_TV_FILE)
        if name in tv:
            return tv[name]
        mp = self._read(_MP_FILE)
...
MAX_FIX_RETRIES = 5  # OpenAI model for 1-2, OpenAI model for 3+
...
# rather than raising an exception. The Python kernel (WS) stays alive â€” only Spark is down.
...
# â”€â”€â”€ OCI / Download â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ OCI Object Storage Fallback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def upload_to_object_storage(local_file: str, object_name: str, log_fn=None) -> bool:
    ...
    try:
        ...
        if log_fn:
            log_fn(f"  [OCI backup] FAILED: {object_name} â€” {str(e)[:200]}")
        return False
...
def fallback_upload_to_ocs(local_files: list, output_dir: str,
    ...
    if log_fn:
        log_fn(f"  [OCI backup] {'Complete' if all_ok else 'Partial â€” some uploads failed'}")
    return all_ok
...
def _download_path_variants(path: str) -> list:
    ...
    Try all combinations of {as-is, +.ipynb, -.ipynb} Ã— {as-is, spacesâ†”underscores}.
    ...
    if path.endswith(".ipynb"):
        add(path[:-6])
    # 4. Spaces â†’ underscores (and combinations)
    if " " in path:
        u = path.replace(" ", "_")
        add(u)
        if not u.endswith(".ipynb"):
            add(u + ".ipynb")
    # 5. Underscores â†’ spaces (try if no original spaces)
        ...
        for retry in range(max_retries):
            try:
                ...
                if retry < max_retries - 1:
                    ...
                    time.sleep(wait)
                continue
    print(f"[download] FAILED â€” all {len(variants)} path variants exhausted for {path!r}: tried {variants}")
    return None
...
# â”€â”€â”€ OpenAI Responses API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

_CLIENT = None
def model_client():
    global _CLIENT
    if not _CLIENT:
        _CLIENT = OpenAIModelCompatClient(api_key=OPENAI_API_KEY or None)
    return _CLIENT
...
# â”€â”€â”€ Cell History Replay â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def _replay_cell_entry(entry: dict, why: str, session, log_fn=None) -> dict:
    ...
    Injects `why` into fix context so OpenAI model understands the rewind reason.
    ...
    for attempt in range(MAX_REPLAY_RETRIES):
        # Write-redirect on replay too â€” replay re-executes a previously-
        ...
        exec_code = _inject_write_guard(exec_code)  # variable write dests â†’ tmp
        try:
        ...
        # Check for notebook.exit() â€” treat as successful early stop.
...
# â”€â”€â”€ Session Pool â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Monitor / Circuit Breaker â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

async def monitor_analyze_failures(errors: List[str], notebook_path: str) -> Tuple[str, str]:
    """Ask OpenAI model whether failures are systemic or notebook-specific.
...
# â”€â”€â”€ Cell Migration Prompt â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

CELL_MIGRATE_PROMPT = """You are migrating a single Databricks cell to OCI AIDP (Python 3.11).

CRITICAL â€” NO EXECUTION-TIME SCAFFOLDING IN THE MIGRATED NOTEBOOK:
...
logic with these EXPLICIT substitutions applied â€” and nothing else:
  - dbutils.* â†’ oidlUtils.* / aidp_compat.* (API renames listed below)
  - s3://, s3a://, dbfs:// â†’ OCI paths via translate_path / suggest_oci_path
  - %run / notebook.run paths â†’ migrated-path equivalents (MIGRATED DEPENDENCY PATHS)
  - Unsupported APIs â†’ commented out with "# Oracle tool modification: <reason>"
...
gets persisted to the saved notebook forever. Specifically FORBIDDEN:
  ...
  - **TABLE READS STAY AS TABLE READS â€” UNCONDITIONAL.** spark.read.table(X),
    ...
    supports Unity Catalog identifiers identically to Databricks â€” the table read
    ...
    and let the original Spark error fire â€” do not invent a parquet/path rescue.
  ...
  - Inlining the body of a dbutils.notebook.run() / oidlUtils.notebook.run() target into
    the calling cell. If the call fails, fix the path or arguments â€” never paste the
  ...
  - sys.path.insert/append, !pip install, %pip install in cell code â€” these go in
...
the cell's contents with "# Oracle tool modification: <reason>" â€” but the cell itself
...
migrate a cell's logic, comment all of it out â€” never delete.

CRITICAL â€” Path rewriting safety for %run / dbutils.notebook.run / oidlUtils.notebook.run:
...
2. When matching a `%run` token (e.g., `M90`) against the MIGRATED DEPENDENCY PATHS list,
   match by EXACT basename equality only â€” never by prefix. A `%run M90` lookup must
...
3. Every migrated `%run` path must end in exactly one ".ipynb" suffix. Patterns like
   ...
   construction bug â€” never emit such paths.

CRITICAL â€” AWS / boto3 / Glue dependencies have NO AWS SDK on AIDP:
...
equivalent â€” DO NOT comment out as "unused". "Unused-in-this-notebook" is NEVER safe because
...
Common patterns:
  - boto3 Glue table location lookups â†’ spark.sql DESCRIBE FORMATTED (see Glue example below)
  - boto3 S3 read/write â†’ spark.read.X(oci://...) / use suggest_oci_path tool first
  - boto3 SecretsManager â†’ aidp_compat.secrets or OCI Vault
For services not listed, define a local AIDP equivalent inline using Spark catalog,
aidp_compat, or OCI SDK â€” keep the function signature identical so callers don't break.
...
AIDP â€” BUT note the result schema differs and the downstream access must change too:
  ...
  DESCRIBE DETAIL    â†’ returns 1 Row with NAMED fields:  row['location'], row['format'], â€¦
  DESCRIBE EXTENDED  â†’ returns N Rows with col_name/data_type/comment columns;
...
Rewrite pattern (apply BOTH the SQL and the result-access change together):
  ...
  # After  -- Oracle tool modification: DESCRIBE DETAIL â†’ DESCRIBE EXTENDED (AIDP-safe)
  rows = spark.sql(f"DESCRIBE EXTENDED {tbl}").collect()
  path = next(r['data_type'] for r in rows if r['col_name'] == 'Location')

Apply this rewrite IN-PLACE wherever you find `DESCRIBE DETAIL` â€” both inside helper
...
`DESCRIBE EXTENDED`, `DESCRIBE FORMATTED`, or `DESCRIBE` (plain) â€” only `DESCRIBE DETAIL`.
...
- OCI Python SDK auth: API key via CLI config file at /Workspace/<oci-config-workspace-path>
  ...
  failure modes on AIDP and MUST NEVER be used. If source code already uses the API-key init
  pattern (oci.config.from_file pointing under /Workspace/), PRESERVE that init code unchanged.
- All configured migration libraries are on classpath

CRITICAL â€” NEVER use direct JVM Hadoop FileSystem calls in migrated notebooks:
The following patterns FAIL in source workload's scheduled workflow runs and MUST NOT appear in any
migrated cell:
  ...
  fs.open(p) / fs.exists(p) / fs.listStatus(p) / fs.delete(p) / fs.rename(p) â€” when `fs` came
...
JVM class isn't initialized the same way at scheduled-job execution time. Use these instead:
  - Spark APIs for tabular data: spark.read.parquet("oci://...") / df.write.parquet("oci://...")
    These ARE safe â€” Spark wires BmcFilesystem internally for SQL/DataFrame ops.
...
If you migrate boto3/S3 code to OCI, use the OCI Python SDK â€” NEVER substitute with direct
jvm Hadoop FS calls (that's the trap OpenAI model fell into in past runs). If existing source code
...
- %run is supported on AIDP â€” keep %run cells as-is, do NOT convert to dbutils.notebook.run().
...
- dbutils.notebook.run() -> oidlUtils.notebook.run() (AIDP native equivalent)
  ...
  On AIDP you MUST pass a positive value â€” use 3600 (or the max supported).
  ...
  Do NOT parse the result inline. Capture it first and parse DEFENSIVELY â€” guard
  ...
  swallows them â€” real errors must propagate.
  ...
  Map("dateStr"->d) not Map("arg1"->d)) â€” read the child with read_notebook_source
...
- dbutils.notebook.exit() -> oidlUtils.notebook.exit() (AIDP native equivalent)
  ...
  execution flow â€” if disabled, all downstream cells execute in a code path that was supposed to be
...
- dbutils.widgets.text/dropdown/combobox/multiselect: Comment out these widget registration calls.
  They are not needed in AIDP â€” workflow parameters come through oidlUtils.parameters.
- oidlUtils is a NATIVE AIDP module â€” it is pre-loaded in every kernel. Do NOT import it.
  No "from aidp_compat import oidlUtils" or "import oidlUtils" â€” just use oidlUtils.xxx directly.
- TRINO / ATHENA (pyathena) â†’ AIDP SPARK CATALOG: AWS Athena and Trino/Presto are external
  ...
  â€” read via Spark, KEEPING the SQL text:
    from pyathena import connect / import trino / as_pandas / try-except !pip install  â†’ comment out
    conn = connect(...) / trino.dbapi.connect(...)                                      â†’ remove
    pd.read_sql(QUERY, conn)         â†’ spark.sql(QUERY).toPandas()   # keep pandas shape
    cur.execute(QUERY); cur.fetchall() â†’ spark.sql(QUERY).collect()
    as_pandas(cursor)                â†’ spark.sql(QUERY).toPandas()
  Table names in QUERY: keep schema.table (resolves in catalog); for trino catalog.schema.table
  DROP the leading trino catalog â†’ schema.table. This is a TABLE READ (never a path read). The
  QUERY is Presto/Trino SQL â€” most ANSI SQL runs as-is in Spark SQL; translate engine-specific
  functions (approx_distinctâ†’approx_count_distinct, cardinalityâ†’size; great_circle_distance has
  no Spark builtin â†’ reimplement via Haversine) or make_note. If pyathena/trino are imported but
  never used to query (dead import), just comment the import + make_note â€” do NOT install/connect.
...
- If a notebook path is NOT in the migrated dependency list, do NOT call it AND do NOT
  ...
  forbidden â€” it produces an opaque migrated notebook and divergent execution semantics.
...
- AIDP does not support spaces in paths. Replace spaces with underscores in ALL /Workspace/ paths
  in the code â€” %run paths, file open() paths, string literals referencing /Workspace/ directories, etc.
  (e.g. "/Workspace/Users/foo/Model Monitoring/Utils" â†’ "/Workspace/Users/foo/Model_Monitoring/Utils").

CRITICAL - SLACK / NOTIFICATIONS:
Slack/notification cells are handled automatically â€” they are converted to Raw cell type with original code preserved.
...
(catalog and schema both default to 'default' in the source workload's AIDP environment).

Translation rules â€” apply to ALL path strings in the cell, including f-strings and variables:
  /dbfs/FileStore/x  â†’  /Volumes/default/default/dbfs/FileStore/x
  dbfs:/FileStore/x  â†’  /Volumes/default/default/dbfs/FileStore/x
...
2. PATH LOADED FROM DB/CONFIG (cell_plan risks: dbfs_path_from_config):
   Wrap runtime values with translate_path() â€” handles /dbfs/, dbfs:/, s3://, /mnt/ all at once:
   ...
   AND call make_note("dbfs_path_from_config: path from DB â€” verify translation at runtime")

CRITICAL â€” Path-prefix replacements MUST be idempotent:
NEVER write an unconditional .replace() to translate a path prefix. .replace() runs
on every call and on every occurrence â€” if the same value is fed in twice (e.g. in a
...
You CANNOT do `s = s.startswith("/dbfs/", "/Volumes/...")` â€” that passes the second
...
FORBIDDEN patterns (all produce wrong / mangled output):
  ...
  path = path.startswith('/dbfs/', '/Volumes/default/default/dbfs/')     # TypeError â€” wrong API

CORRECT â€” use one of these forms:

  # Option A â€” startswith guard, then build the new path explicitly:
  if path.startswith('/dbfs/'):
      path = '/Volumes/default/default/dbfs/' + path[len('/dbfs/'):]

  # Option B (PREFERRED) â€” translate_path() from aidp_compat. It is idempotent:
...
unconditional .replace() â€” always guard with .startswith() or delegate to translate_path().
...
DO NOT attempt to fix this in code â€” it requires data infra action.
...
For EMPTY_SCHEMA tables: Table shell registered but DESCRIBE returns 0 columns â€” data
not synced from source catalog. Requires infra investigation.

Action: call make_note("TABLE_BLOCKED: <table_name> is MISSING/EMPTY_SCHEMA â€” cannot
proceed until registered in AIDP catalog"). If ALL table refs in the cell are blocked, raise:
  RuntimeError("AIDP: Table <name> not available â€” add to tables_to_migrate.csv and re-run")
...
- All other paths (/FileStore/, /dbfs/, dbfs:/, /mnt/) are local DBFS paths â€” use translate_path()
...
- Use suggest_oci_path tool ONCE to get the correct OCI bucket name + namespace for any S3 path.
  The suggest_oci_path result is authoritative â€” use it exactly as returned. Do NOT modify the
...
- After mapping, validate the path exists using explore_path or run_on_cluster â€” data may not be migrated yet.
...
- PATH NOT FOUND RULE: If the bucket exists but the sub-path does not, do NOT keep guessing other
  ...
  The data may not be migrated to OCI yet â€” that is OK, just note it and move on.
- CROSS-TENANCY ACCESS: In AIDP, cross-tenancy OCI buckets require a registered table (real or dummy)
  ...
  1. AIDP LOCATION EXTRACTION (for code that returns a LOCATION STRING â€” NOT for data reads).

     SCOPE â€” exactly TWO kinds of sites get touched. Anything else stays byte-identical.

     A) FUNCTION DEFINITION SITES â€” where a helper is `def`-ed:
        ...
        The function STILL RETURNS A PATH STRING â€” just derived from AIDP
        DESCRIBE FORMATTED instead of AWS Glue.

     B) INLINE EXTRACTION BLOCKS â€” where DESCRIBE FORMATTED is written out in
     ...
     ABSOLUTELY FORBIDDEN â€” these are the over-migration mistakes we are
     correcting:
       ...
       - REWRITING A CALL SITE. A line like
         ...
         correct as-is â€” the FUNCTION's body is what gets fixed (case A above),
         not the call.
     ...
     Snippet rules:
       ...
       - POSITIONAL row[0] / row[1] access â€” AIDP's DESCRIBE FORMATTED column
         names differ slightly from Databricks; positional access is robust.
       - CASE-INSENSITIVE match (r[0].strip().lower() == "location") â€” AIDP
     ...
     spark.sql("...FROM...") data reads â€” those stay as table reads (see the
     unconditional rule above).
  2. If search_catalog says a table does not exist, do NOT explore OCI paths for that data â€” direct
...
Databricks pre-imports many symbols into the kernel namespace. AIDP does NOT â€” every symbol
...
Python:
- datetime, timedelta, date â€” add: from datetime import datetime, timedelta, date
- OrderedDict, defaultdict, Counter, namedtuple â€” add: from collections import <name>
- Any pyspark.sql.functions symbol used without F. prefix (col, lit, when, udf, etc.) â€” add: from pyspark.sql.functions import <symbol>
- Any pyspark.sql.types symbol used bare (StringType, IntegerType, StructType, etc.) â€” add: from pyspark.sql.types import <symbol>
- math, re, json, os, sys â€” add explicit import if used but not imported

Scala (%scala cells):
- JSONObject, JSONArray, JSONException â€” add: import org.json.JSONObject (etc.)
- SimpleDateFormat â€” add: import java.text.SimpleDateFormat
- ArrayBuffer â€” add: import scala.collection.mutable.ArrayBuffer
- Base64 â€” add: import java.util.Base64
...
KNOWN MIGRATION PATTERNS â€” fix these directly during migration, no investigation needed:
...
- %pip install: Comment out original with "# AIDP: installed via cluster libraries API" comment.
  Do NOT run pip install via subprocess or run_on_cluster â€” packages are installed automatically
...
- display is available natively on AIDP â€” do NOT import it from aidp_compat.
  ...
  For everything else, use the library's OWN native display function:
    * Spark DataFrame      : `df.display()`     â†’ `display(df)`           (AIDP native)
    * Pandas DataFrame     : `df.display()`     â†’ `df` as last expression (pandas/IPython auto-render)
                              or explicit form  â†’ `print(df.to_string())`
    * matplotlib figure    : `fig.display()`    â†’ `plt.show()`            (matplotlib native)
    * plotly figure        : `fig.display()`    â†’ `fig.show()`            (plotly native)
    * PIL Image            : `img.display()`    â†’ `img.show()`            (PIL native)
    * numpy array / dict   : `arr.display()`    â†’ `print(arr)`            (Python native)
...
CRITICAL - MLFLOW:
  ...
  via the cluster libraries API â€” do NOT run pip install yourself.
  ...
  2. Add mlflow.set_tracking_uri() ONCE in the first cell that references mlflow in any way
     ...
     so notebooks may use it without an explicit import. If there is no `import mlflow`, add it:
       import mlflow
       mlflow.set_tracking_uri("https://<MLFLOW_USER>:<MLFLOW_PASSWORD>@<MLFLOW_HOST>")
     Do not repeat in later cells.
  3. Convert MLflow 2.x deprecated APIs to 3.x equivalents when they fail:
     - get_latest_versions(name, stages=['Production']) â†’ get_model_version_by_alias(name, "production")
     - get_latest_versions(name, stages=['Staging']) â†’ get_model_version_by_alias(name, "staging")
     - Other stage-based APIs â†’ use alias-based equivalents
  Do NOT rewrite MLflow client calls to raw requests.post() REST API calls.
  Do NOT add defensive fallbacks â€” if the API call fails, fix the API call.
  If MLflow tracking fails after 2 attempts, comment it out with:
    # TODO: MLflow â€” disabled for AIDP migration; re-enable after configuring AIDP MLflow tracking server
...
# FUSE evicts the kernel inode cache between reads â€” the second open() fails.
# Rules:
...
# 4. pd.read_csv is generally safe â€” use safe_read_file only for bare open() calls
#
...
# Spark lazy evaluation problem â€” inject safe_materialize() before write on flagged DataFrames.
...
EXCEPTION â€” %scala cells that use the Cassandra/ScyllaDB Spark connector (any of:
...
spark.sql / Spark DataFrame APIs. See "SCYLLADB â†’ AIDP METASTORE MIGRATION" section
below for exact substitutions.

=== SCYLLADB â†’ AIDP METASTORE MIGRATION (Cassandra-bearing %scala cells only) ===
...
name. Reference shape:  spark.sql("â€¦  scylla_$keyspace.$table  â€¦").
Concrete example: Scylla keyspace `sample_keyspace` â†’ AIDP metastore schema
`scylla_sample_keyspace`; table `foo` â†’ `scylla_sample_keyspace.foo`.

THIS IS A TEMPORARY MAPPING. Mark every migrated cell with this header (immediately
after the %scala magic line):

  // Oracle tool modification: ScyllaDB â†’ AIDP metastore (temporary).
...
REMOVE entirely:
  ...
  CassandraConnector(spark.sparkContext).withSessionDo { session => BODY }
    â†’ inline BODY at the top level (no wrapper); reference `spark` directly
...
REPLACE â€” reads:
  ...
  â†’ spark.sql(s"SHOW TABLES IN scylla_$ks").collect().map(_.getString(1))   // col 1 = tableName

  session.execute(s"SELECT col1, col2 FROM $ks.$t WHERE â€¦")
  â†’ spark.sql(s"SELECT col1, col2 FROM scylla_$ks.$t WHERE â€¦").collect()    // or .toDF() for DataFrame chain

  session.execute(simpleStatement)                       // SimpleStatement-wrapped CQL
  â†’ strip the SimpleStatement(...) wrapper; pass the bare query string to spark.sql(...) (prefix the schema with scylla_)

  spark.read.format("org.apache.spark.sql.cassandra")
       .options(Map("keyspace" -> ks, "table" -> t)).load()
  â†’ spark.read.table(s"scylla_$ks.$t")

REPLACE â€” writes (ALWAYS use mode("overwrite") â€” see Write-Mode rule below):
  session.execute(s"INSERT INTO $ks.$t (col1, col2) VALUES ('a', 1)")
  â†’ spark.sql(s"INSERT OVERWRITE scylla_$ks.$t VALUES ('a', 1)")             // overwrite, not append

  session.execute(s"UPDATE $ks.$t SET col=val WHERE pk=?", bindings)
  â†’ spark.sql(s"UPDATE scylla_$ks.$t SET col=val WHERE pk=â€¦")                // Delta only; call make_note if non-Delta

  session.execute(s"DELETE FROM $ks.$t WHERE â€¦")
  â†’ spark.sql(s"DELETE FROM scylla_$ks.$t WHERE â€¦")                          // Delta only; call make_note if non-Delta

  df.write.format("org.apache.spark.sql.cassandra")
        .options(Map("keyspace" -> ks, "table" -> t)).save()
  â†’ df.write.mode("overwrite").saveAsTable(s"scylla_$ks.$t")                 // ALWAYS overwrite â€” temporary mapping
                                                                              // add Write-Mode comment (see below)

REPLACE â€” DDL:
  session.execute(s"DROP TABLE $ks.$t")
  â†’ spark.sql(s"DROP TABLE IF EXISTS scylla_$ks.$t")                         // ALWAYS add IF EXISTS

  session.execute(s"TRUNCATE $ks.$t")
  â†’ spark.sql(s"TRUNCATE TABLE scylla_$ks.$t")

  session.execute(s"CREATE TABLE $ks.$t (col1 TYPE, col2 TYPE, PRIMARY KEY (col1)) WITH â€¦")
  â†’ spark.sql(s"CREATE TABLE IF NOT EXISTS scylla_$ks.$t (col1 TYPE, col2 TYPE) USING DELTA")
    // drop CQL-specific PRIMARY KEY / clustering / WITH options; preserve column types

PreparedStatement / BoundStatement:
  val ps = session.prepare("INSERT INTO â€¦"); session.execute(ps.bind(a, b))
  â†’ strip the prepare/bind dance; build the SQL string and pass to spark.sql(...) (prefix schema with scylla_)
  Prefer DataFrame writes for bulk inserts: bind values â†’ Row â†’ spark.createDataFrame(...).write...

Batch statements:
  BEGIN BATCH stmt1; stmt2; â€¦ APPLY BATCH
  â†’ unroll: spark.sql(stmt1); spark.sql(stmt2); â€¦ (no transactional guarantee â€” acceptable for temporary mapping)
...
WRITE-MODE rule (mandatory for ALL writes to a Scylla-mirrored table):
  ...
  deterministic results is overwrite. Mark every write with this exact inline comment:
    ...
    // Temporary â€” restore original semantics when Scylla becomes directly reachable on AIDP.

EXISTENCE GUARDS â€” mandatory; inject before any operation that touches scylla_$ks.*:
  - Before SHOW TABLES IN scylla_$ks / any read on scylla_$ks.* / any DDL on scylla_$ks.*:
      ...
      if (!_schemas.contains(s"scylla_$keyspace")) {
        println(s"AIDP: schema 'scylla_$keyspace' not in metastore â€” skipping cell.")
      } else {
        // ... migrated body ...
      }
  ...
  - For per-table reads/writes/updates/deletes (operating on a known table name), wrap with:
      ...
      if (!_tables.contains(tableName)) {
        println(s"AIDP: table scylla_$ks.$tableName not present â€” skipping")
      } else {
        // ... read/write logic ...
      }

CQL-SPECIFIC SYNTAX HANDLING:
  - ALLOW FILTERING                              â†’ drop the clause (Spark scans by default)
  - USING TTL / USING TIMESTAMP                  â†’ drop and call make_note (no Spark equivalent)
  - IF EXISTS / IF NOT EXISTS lightweight txn    â†’ drop and call make_note (not supported in Spark)
  - BEGIN BATCH â€¦ APPLY BATCH                    â†’ unroll to sequential spark.sql(â€¦) calls
  - Materialized views / secondary indexes / UDTs â†’ call make_note for manual review
...
=== END SCYLLADB â†’ AIDP METASTORE MIGRATION ===
...
- Do NOT remove or stub code because you think a variable is "unused" or "dead code".
  ...
  Translate the code as-is â€” path translation, API changes â€” nothing more.
- Do NOT replace file reads (pickle.load, open, pd.read_csv, etc.) with hardcoded values.
  If the file path needs translation (e.g. /dbfs/ â†’ /Volumes/), translate the path and keep the read.
- Do NOT invent problems that don't exist. Do NOT add workarounds for hypothetical issues
  ...
  execution that requires it. FUSE issues are resolved on AIDP â€” write directly to /Volumes.
- Do NOT add defensive guards, null checks, or error handling that the original code didn't have.
  If the original code does `datay[:, 0][:5]`, keep it as-is â€” do NOT wrap it in shape checks.
...
- Python/pandas version upgrades are OK (e.g. df.append â†’ pd.concat for pandas 2.x)
...
- No TODO comments, no placeholder code, no "implement later" stubs
  (EXCEPTION: unfixable Databricks infrastructure â€” see UNFIXABLE DATABRICKS INFRASTRUCTURE section above)
...
EXECUTION CONTEXT â€” READ BEFORE USING TOOLS:
...
- fixup_cell(start_index, why, fixed_code): rewind to a history index and replay all cells from there to current through the full execute+verify+fix loop. IDEMPOTENCY WARNING: replay re-executes every cell in the sequence â€” only use when cells are idempotent (no double file writes, no duplicate external calls).
...
or DataFrame that downstream cells depend on. If yes, you MUST find a working AIDP equivalent â€”
...
downstream â€” and even then, replace with print() confirming what was skipped.
...
- boto3.client('s3') â€” ONLY when code uses boto3/botocore directly for S3 object access
  ...
  that can use oci:// paths directly â€” for those, just change the path to oci://.

  AWS S3 is NOT accessible from AIDP â€” no AWS credentials. Convert direct boto3 S3 calls to
  ...
  ENTIRE function body â€” do not just change the caller. The function itself must use OCI SDK.
...
- s3a:// or s3:// paths in spark.read/spark.write: Use suggest_oci_path to translate to oci://bucket@namespace/key.
  These do NOT need OCI SDK â€” Spark can read/write oci:// paths natively.
- spark.read.format("org.apache.spark.sql.cassandra"): Use explore_path/describe_table to check
  ...
  RuntimeError("AIDP: Cassandra source not migrated â€” data at <source> not found in OCI catalog").
  NEVER silently return empty data.
- spark.read.format("hudi"): Keep the format â€” Hudi is supported on AIDP. Add comment:
  # AIDP: requires a compatible Hudi Spark bundle JAR in cluster libs + spark.serializer=KryoSerializer

SIDE-EFFECT-ONLY CALLS (safe to comment out â€” these produce NO data):
...
- JobRunAPI(...) / requests.post to api/2.0/jobs/run-now:
  Comment out original code, add: print("AIDP: Skipped Databricks job trigger â€” not applicable on OCI")
- Private/internal HTTP calls (notifications/pings only):
...
- %pip install cells: Comment out original with "# AIDP: installed via cluster libraries API" comment.
  Do NOT run pip install via subprocess or run_on_cluster â€” the migration tool auto-installs packages.

UNFIXABLE DATABRICKS INFRASTRUCTURE (stub on first attempt â€” do NOT retry):
...
Pattern:
  ...
  print("STUBBED: Databricks job trigger â€” original code preserved in comments above")
...
- Use make_note("STUBBED: <what> â€” <why>") so the stub is tracked in migration notes
...
- Table references: AIDP uses 3-part names (catalog.schema.table) with "default" as the catalog.
  ...
  Tables match exactly or they don't exist â€” do NOT explore, search variations, or guess names.
  If a table appears in VERIFIED TABLES as EXISTS, trust it â€” do NOT re-check.
...
- DELETE FROM / UPDATE / MERGE statements: Keep code as-is â€” do NOT rewrite.
  These are source logic and must be preserved exactly. Skip execution during migration
...
- dbutils.notebook.entry_point.getDbutils().notebook().getContext().toJson():
  The aidp_compat shim already handles this â€” dbutils.notebook.entry_point is mocked.
...
- /Volumes path in os.makedirs: Keep the full /Volumes/... path â€” never strip the prefix
...
%python
# AIDP performance configuration â€” applied automatically by migration tool
# Some configs are cluster-level static (e.g. maxResultSize) â€” wrap all in try/except
for _k, _v in [
    ("spark.sql.execution.arrow.pyspark.enabled", "false"),
    # spark.driver.maxResultSize is static â€” set at cluster level, not at runtime
    ...
    try:
        ...
        pass  # static config â€” set at cluster level
"""
...
# must skip it when re-loading such a dep â€” otherwise child_cells would include
...
# â”€â”€ Data-recovery override block markers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
# saved to disk â€” the saved notebook must contain the source workload's ORIGINAL
# date filter, not our test-only substitution.
_DATA_RECOVERY_BEGIN = "# === AIDP_DATA_RECOVERY_OVERRIDE_BEGIN (test-only â€” not saved) ==="
...
# (fail-safe â€” never accidentally strip user code).
...
def _strip_data_recovery_block(source: str) -> str:
    ...
    the source workload's saved notebook. Safe to call on sources that don't have
    the block â€” it's a no-op in that case. Fail-safe: requires both markers
...
# â”€â”€ Empty-data failure classifier â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
# instead of burning OpenAI model retries trying to "fix" the unfixable.
...
def _is_empty_data_failure(output: str, exec_code: str) -> bool:
    ...
    likely a real bug (not a data issue) â€” don't classify as empty-data.
    """
    if not output:
        return False
...
# â”€â”€ Write-cell heuristic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
# any write-like signature â†’ not a candidate for data recovery.
...
def _is_write_cell(source: str) -> bool:
    ...
    cell's active (uncommented) source. Conservative â€” better to skip
...
# â”€â”€ Write-redirect (tool-only): never touch production data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
# a TOOL-OWNED destination so production data is never touched.
# The SAVED notebook keeps the source workload's original paths/tables intact;
# only the cluster-executed code (`exec_code`) is rewritten.
#
# Redirect targets:
#   â€¢ Tables: default.oracle_ai_automation_overwrite.<db>_<tbl>
#             where <db>_<tbl> comes from canonicalizing the source workload's
#             identifier (1-/2-/3-part â†’ 3-tuple â†’ schema_table). Catalog
#             is dropped because AIDP only uses "default" anyway.
#   â€¢ Paths:  oci://oracle_ai_automation@<WORKSPACE_NAMESPACE>/<orig-suffix>
#             where <orig-suffix> is everything after `bucket@ns/` in the
#             original â€” preserves file/folder structure for predictability.
#
# Read semantics (Option A â€” data isolation):
#   â€¢ A read of `T` consults the redirect map. If a prior cell in this job
#     wrote to `T`, the read substitutes to `T'` (the redirect target).
#   â€¢ If `T` was NEVER written by the tool in this job, the read is left
#     as-is (reads source workload's production â€” non-destructive).
#   â€¢ Net effect: tool never WRITES to production. Reads of "not-yet-
...
#     (not the union with prior production data). Operators accept this
#     because at runtime the original code reads/writes production
#     as intended.
#
# Scope:
#   â€¢ Map is module-level and persists across tasks within the same job.
#     Reset between jobs via `clear_write_redirect_map()`.
#   â€¢ Skip `/Volumes/...`, `/Workspace/...` (AIDP-local FUSE; not OCI).
#   â€¢ Skip targets already in the tool namespace (idempotency on retries).
...
def _canonicalize_table_id(raw: str) -> Optional[Tuple[str, str, str]]:
    """Normalize a 1/2/3-part table identifier to (catalog, db, tbl).
      ...
      "tbl"             â†’ ("default", "default", "tbl")
      "db.tbl"          â†’ ("default", "db",      "tbl")
      "catalog.db.tbl"  â†’ ("catalog", "db",      "tbl")
...
async def _ensure_redirect_schema(session) -> None:
    ...
    connection (session.cluster_id set) â€” it executes CREATE SCHEMA on the
    ...
    if cluster_id is None:
        ...
        raise ValueError(
            ...
            "per-task connect â€” the data-safety contract depends on the "
    ...
    # raising yet leave the schema absent (observed on AIDP â€” the call silently
    ...
    # fallback; if the schema STILL doesn't exist, raise â€” a missing redirect
...
def _canonicalize_path(raw: str) -> Optional[str]:
    ...
    stripped â€” two writes to byte-identical strings collide on the same
...
def _redirect_target_for_path(orig: str) -> Optional[str]:
    ...
    in different buckets share an identical suffix:
      ...
      oci://source-prod@ns1/sales/2024/  â†’  oci://oracle_ai_automation@<WORKSPACE_NAMESPACE>/source-prod/sales/2024/
      oci://other-prod@ns2/sales/2024/ â†’  oci://oracle_ai_automation@<WORKSPACE_NAMESPACE>/other-prod/sales/2024/
...
def _is_already_redirected_path(raw: str) -> bool:
    ...
    tool's own namespace â€” don't redirect again."""
...
# are intentionally LITERAL â€” dynamic destinations (f-strings, variables)
...
# .option("path", "oci://...") in a writer chain â€” captures the path.
...
# SQL DDL/DML â€” matches the table identifier following the keyword.
...
# SQL FROM <table>, JOIN <table>, INTO <table> (for SELECT side of MERGE) â€”
# captures reads.
...
def _record_redirect(kind: str, original: str, redirected: str, source_op: str) -> None:
    ...
    if kind == "table":
        # Check if any prior entry maps to the same redirected target â€”
        ...
        if len(collisions) > 1:
            _write_redirect_collisions.append({
                ...
                "note": "Multiple source tables redirect to the same target â€” last writer wins.",
            })
    elif kind == "path":
        ...
        if len(collisions) > 1:
            _write_redirect_collisions.append({
                ...
                "note": "Multiple source paths redirect to the same target â€” last writer wins.",
            })
...
def _apply_wrapper_call_redirect(exec_code: str, source_op_hint: str = "") -> str:
    """AST-based cell-text rewrite of source writer-wrapper function calls.

    Reads reports/writer_wrappers.json â€” for every Call node whose function
    ...
    Why this exists (and replaces the older runtime monkey-patch approach):
      ...
      - Saved source code is NOT modified â€” rewrite is applied only to
        the exec_code sent to the kernel, NOT the cell source written back.

    Behaviour:
      - Unparseable code â†’ return unchanged (don't crash the cell).
      - No catalog â†’ return unchanged (graceful degradation).
    ...
    def _redirect_for_role(role: str, current: str) -> Optional[str]:
        ...
        if role == "full_id":
            # 'tbl' / 'db.tbl' / 'cat.db.tbl' â†’ rewrite db slot to temp.
    ...
    try:
        ...
        # ast.unparse failure on some pathological tree â€” skip rewrite
    ...
    # â€” which is the read-side requirement.)
    return new_code


# â”€â”€ Runtime write-destination guard (catches VARIABLE write targets) â”€â”€â”€â”€â”€â”€
...
# runtime helper that maps any oci://bucket@ns/suffix â†’
...
# EXEC time. Works for literals AND variables, in Scala and Python. EXEC-only â€”
# never saved.
...
def _wrap_write_dests(code: str, is_scala: bool) -> str:
    ...
    pw = "_aidp__write_guard__"  # low-collision name so source code can't shadow it
    ...
    for open_idx in sorted(set(opens), reverse=True):
        ...
        if not s or pw + "(" in inner:
            continue
        # For .option("path", X): inner == '"path", X' â€” wrap only the value.
...
def _apply_write_redirects(exec_code: str, source_op_hint: str = "") -> str:
    ...
    â€” callers pass exec_code, not current_code).
    """
    if not exec_code:
        return exec_code

    # Wrapper-call redirect runs FIRST: rewrites source-wrapper call
    ...
    for m in list(_WRITER_TABLE_RE.finditer(exec_code)):
        ...
        # quote style. Use string replace (safe â€” ident is captured from
    ...
    # â”€â”€ Python writer .option("path", "...") â”€â”€
    ...
    #   Comment-only suppression broke `spark.sql("DROP TABLE ...")` calls â€”
    ...
    #   source workload's real table is untouched and our temp table cleans up.
...
def _apply_read_redirects(exec_code: str) -> str:
    ...
    Targets NOT in the map are left as-is â€” they read from the source workload's
    ...
    # SQL FROM/JOIN â€” substitute identifiers that have a redirect entry.
...
# but DO NOT catch source utility wrappers like
#   createTable(df, table_name="x", database_name="real_db", ...)
# because the actual write is hidden inside the function body.
#
# This block installs **runtime monkey-patches** on the source workload's
...
# before invoking the original. Saved source code is NOT modified â€”
# this is in-kernel only, for the duration of tool execution.
#
# The catalog lives in reports/writer_wrappers.json and lists:
#   - actionable: wrappers with explicit (arg_name â†’ role) mapping
#                 where role âˆˆ {table, db, bucket, path, full_id, mode}
#   - refuse:     wrappers that write to hardcoded targets and CANNOT
#                 be safely redirected â€” the interceptor raises if any
#                 of these are called.
#
# Roles redirected by the interceptor:
#   - db    â†’ "oracle_ai_automation_overwrite"
#   - bucketâ†’ "oracle_ai_automation"
#   - path  â†’ oci://oracle_ai_automation@<WORKSPACE_NAMESPACE>/<orig-bucket>/<suffix>
#   - full_id (e.g. "db.tbl") â†’ "oracle_ai_automation_overwrite.tbl"
...
def _load_writer_wrappers_catalog() -> dict:
    ...
    try:
        ...
        tprint(f"  [writer-wrappers] catalog unavailable ({type(e).__name__}: {e}) â€” "
...
def _build_writer_interceptor_install_code(catalog: dict) -> str:
    ...
    The returned string is self-contained â€” it imports only stdlib `re`/`inspect`
    ...
    return (
        ...
        "    # bare filenames, and non-OCI URIs unchanged (the source workload's wrapper may\n"
        ...
        "    # 'tbl' / 'db.tbl' / 'catalog.db.tbl' â†’ db slot replaced.\n"
...
assert _CONFIG_MARKER in AIDP_SPARK_CONFIG_CELL, (
    ...
    f"{_CONFIG_MARKER!r} â€” _inline_child_notebook() relies on it to filter "
    f"the config cell when loading dep notebooks from disk."
)

# Object Storage hardening cell â€” applies CircuitBreaker + retry tuning to the
...
# AIDP Object Storage hardening â€” applied automatically by migration tool
try:
    import os as _os
...
# Runtime-only toPandas() safety limit â€” prevents driver OOM during migration execution.
...
try:
    ...
    if not getattr(_ps.DataFrame, '_aidp_topandas_patched', False):
        ...
        _TOPANDAS_MAX_CELLS = 100_000_000  # rows x cols â€” ~800MB at 8 bytes/cell, safe for 64GB driver
...
# â”€â”€â”€ Per-Notebook Logger â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Deterministic Pre/Post Migration Transforms â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
# These run outside OpenAI model to make critical transformations reliable.
# All three are additive: they never delete or reorder existing code.

# â”€â”€ S3 â†’ OCI Path Translation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

# Default tenancy namespace â€” used ONLY as a last resort when a bucket is not
# in the bucketâ†’tenancy mapping. The mapping (config/oci_bucket_tenancy_mapping.json)
...
def _validate_migrated_run_paths(source: str, migrated_base: str) -> list:
    ...
    arise from a legitimate path:
      ...
      2. ".ipynb<digit>.ipynb" mangling (e.g. "M9.ipynb0.ipynb") â€” when a `%run`
         basename was prefix-matched against dep_path_map and the remainder was
         appended verbatim.
      3. Doubled ".ipynb.ipynb" suffix (e.g. "foo.ipynb.ipynb") â€” basename was
         already suffixed, then re-suffixed.

    Returns a list of issue strings (empty when the source is clean).
    Pure inspection â€” does not modify source. Intended for logging/warning
    after OpenAI model produces a migrated cell.
    """
    issues: list = []
...
def _ns_for_oci_bucket(oci_bucket: str):
    ...
    bucketâ†’tenancy mapping (config/oci_bucket_tenancy_mapping.json), or None if
...
def _apply_s3_translations(source: str) -> str:
    """Deterministically replace s3[a]://bucket/path â†’ oci://oci-bucket@namespace/path.

    Rule (per source convention):
      - OCI bucket name = oci-<s3_bucket>
      - Namespace ALWAYS from the bucketâ†’tenancy mapping
        (config/oci_bucket_tenancy_mapping.json) â€” the single source of truth.
    ...
    def _sub(m):
        ...
        oci_b = f"oci-{bucket}"
        # Namespace ALWAYS from the bucketâ†’tenancy mapping; default tenancy ns
        # only as a last resort for buckets absent from the mapping.
        ns    = _ns_for_oci_bucket(oci_b) or _DEFAULT_TENANCY_NS
        # Return ONLY the translated URI â€” no inline "# ..." comment. s3 paths
...
# â”€â”€ OCI namespace correction (mapping is the ONLY source of truth) â”€â”€â”€â”€â”€â”€â”€â”€
#
# RULE (user-mandated): the OCI namespace for a bucket comes ALWAYS from the
# bucketâ†’tenancy mapping (config/oci_bucket_tenancy_mapping.json) â€” never from
# the source notebook, never from OpenAI model. The exported/source notebooks often
# carry a WRONG namespace (e.g. oci-source-feature-bucket tagged
...
def _apply_namespace_from_mapping(code: str) -> str:
    ...
    bucket. The mapping is authoritative; source/OpenAI model namespaces are ignored.
    ...
    try:
        ...
        # do NOT silently pass code through â€” that lets wrong namespaces survive
        # while the run still reports PASS. Fail loud and stop.
        tprint(f"  [namespace] FATAL: bucketâ†’namespace mapping failed to load ({e!r}); "
               f"cannot guarantee oci:// namespaces â€” aborting to avoid wrong-namespace writes.")
        raise RuntimeError(
            "bucketâ†’namespace mapping unavailable; refusing to proceed (would risk "
            "wrong-namespace writes). Check config/oci_bucket_tenancy_mapping.json."
        ) from e
    if not _BUCKET_NS_INDEX:
        tprint("  [namespace] FATAL: bucketâ†’namespace mapping loaded but is EMPTY; "
               "cannot guarantee oci:// namespaces â€” aborting to avoid wrong-namespace writes.")
        raise RuntimeError(
            "bucketâ†’namespace mapping is empty; refusing to proceed (would risk "
            "wrong-namespace writes). Check config/oci_bucket_tenancy_mapping.json."
        )
    def _sub(m):
...
# â”€â”€ notebook.run timeout fix â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# 2nd positional arg: notebook.run(<first-arg>, 0 <,|)>  â€” first arg may be a
...
def _fix_notebook_run_timeout(source: str) -> str:
    """Rewrite notebook.run timeout 0 â†’ positive (AIDP rejects 0/null)."""
...
# â”€â”€ notebook.run INLINE support (treat like %run) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# notebook.exit value into a var, and substitute the call â†’ that var for
...
def _find_notebook_run_calls(src: str) -> list:
    ...
    for m in _NB_RUN_CALL_RE.finditer(src):
        ...
        while j < len(src):
            ...
            j += 1
        if depth != 0:
            continue  # unbalanced â€” skip (don't mangle)
...
def _parse_run_map_args(full_call: str) -> list:
    ...
    in-scope expression (e.g. prevDayDateStr) â€” used to set the child's params."""
    pairs = []
...
# notebook.exit(...) â€” balanced-paren, for the EXEC-only capture rewrite.
...
def _rewrite_exit_to_capture(cell_src: str, conf_key: str) -> str:
    """EXEC-ONLY: rewrite a child cell's notebook.exit(X) â†’ publish X to
    ...
    child keeps notebook.exit â€” this rewrite is applied only to the exec copy."""
...
# â”€â”€ Validation-only param injection (EXEC-only, never saved) â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
# EXECUTION TIME. The persisted cell is UNCHANGED â€” it keeps
...
# NEVER saved. It deliberately does NOT touch dbutils.widgets.get â€” that must
# still fail â†’ be migrated to oidlUtils by the fix loop (and saved).
...
def _scala_param_exec_rewrite(code: str) -> str:
    """EXEC-ONLY: Scala oidlUtils.parameters.getParameter("X"[, d]) â†’
...
# â”€â”€ DBFS-prefix path-replace idempotency fix â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def _fix_path_replace_idempotency(source: str) -> str:
    """Fix non-idempotent / API-misuse path-prefix substitutions.

    OpenAI model occasionally emits two broken patterns when translating /dbfs/ or
...
# â”€â”€ Table-read â†’ path-read regression detector â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# (The lookbehind allows the call to be on any object â€” spark, df, sql_ctx, etc.)
_TABLE_READ_RE = re.compile(
    r"\.(?:read\s*\.\s*table|table)\s*\(\s*[\"']",
    re.IGNORECASE,
)

# Pattern to detect path-based reads that OpenAI model uses to "rescue" table reads:
...
#
# Lowercased for case-insensitive matching. Conservative â€” false positives just
# fall back to the existing OpenAI model migration path (no harm); false negatives are
...
_DATABRICKS_MARKERS = (
    ...
    # notebook needs the explicit import â€” easier to let OpenAI model add it)
    "displayhtml(",
    ...
    # Must NOT be passed through AS_IS â€” OpenAI model needs to rewrite to DESCRIBE EXTENDED
...
def _has_databricks_markers(source: str) -> bool:
    ...
    optimization â€” if no markers are found, the cell is tried on the cluster
    without an upfront OpenAI model call. Comments are stripped first."""
    if not source:
        return False
...
def _detect_table_to_path_regression(original: str, migrated: str) -> str:
    ...
    into the same cell). Skips %scala / %sql cells â€” Python heuristic only.
    ...
    if (n_orig_table > 0
        ...
        warning = (
            "# Oracle tool modification: REGRESSION_DETECTED â€” tableâ†’path "
...
# â”€â”€ Path-returning-call â†’ catalog-identifier regression detector â”€â”€â”€â”€â”€
...
# whose names declare an intent to hold a path/location string â€” downstream
# code that uses these as filesystem paths is what breaks when OpenAI model replaces
...
# No slashes, no colons â€” that's the tell that it's NOT a path string.
...
def _detect_path_returning_to_identifier_regression(original: str, migrated: str) -> str:
    """Soft-warning detector for the over-migration pattern where OpenAI model
    replaces a path-returning function call with a hardcoded catalog identifier:

        # original (path-returning call):
        events_table_path = get_glue_table_s3_location('analytics', 'events_data')

        # migrated (hardcoded catalog ident â€” wrong):
        events_table_path = "analytics.events_data"
    ...
    literal with no slashes, flag as REGRESSION_DETECTED. Soft warning only â€”
    ...
    for var, orig_val in orig_vars.items():
        ...
        if "/" in content or ":" in content:
            continue  # not a catalog ident â€” looks like a path/URI
        if _CATALOG_IDENT_RE.match(content):
            regressed.append((var, orig_val[:60], content))

    if regressed:
        tprint(f"[regression-check:pathâ†’ident] FIRED â€” {len(regressed)} regressed var(s): {[r[0] for r in regressed]}")
        details = "; ".join(
            f"{var} (was '{call}' â†’ now \"{ident}\")"
            for var, call, ident in regressed[:3]
        )
        warning = (
            f"# Oracle tool modification: REGRESSION_DETECTED â€” path-returning "
...
# â”€â”€ Deterministic rename-only Databricks â†’ oidlUtils API rewrites â”€â”€â”€â”€â”€
#
...
# NOT depend on a runtime crash to trigger an OpenAI model rewrite. Unknown/exotic call
...
# /tmp shim does NOT propagate values across them â€” only oidlUtils.parameters
...
def _classify_args(args: list, kw_names: set):
    ...
    for a in args:
        ...
        # A leading "name=" is a kwarg only if name is a known parameter â€” this
...
def _reshape_taskvalues_set(args: list):
    """dbutils.jobs.taskValues.set(key, value) â†’ 'key, value' arg string, or None."""
...
def _reshape_taskvalues_get(args: list):
    """dbutils.jobs.taskValues.get(taskKey, key, default, debugValue) â†’
    ...
    # by name only â€” consistent with CELL_MIGRATE_PROMPT's .getâ†’getParameter rule).
...
# â”€â”€ Internal Notebook Path Rewriting â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _rewrite_internal_paths(source: str, migrated_base: str, notebook_path: str = "",
    ...
    Applied per-cell AFTER OpenAI model migration, BEFORE execution.
    ...
    4. Normalises remaining %run paths: spaces â†’ underscores, adds .ipynb.
    """
    ...
    def _lookup_dep_map(raw_path: str) -> str:
        ...
        Tries (in order):
          1. Full normalized match (strip /Workspace/, spacesâ†’_, lowercase)
          2. Suffix match â€” raw path ends with a known dep key (catches relative
             paths like './Config/0_Config' against absolute dep_map keys)
          3. Basename match â€” last path segment alone (catches bare-name %run
             where the notebook name is unique within the job)
        """
        # Normalize for comparison: strip /Workspace/, ./, ../, spacesâ†’underscores
        def _norm(p):
            p = p.strip().replace(" ", "_")
            # Strip leading ./ and ../ segments â€” keep the meaningful suffix
        ...
        # 2. Suffix match â€” orig ends with raw_norm or vice-versa
        for orig, mig in _dep_map.items():
            if not mig:
                continue
            on = _norm(orig)
    ...
    for old in _OLD_NB_BASES:
        if old in source:
            ...
            tprint(f"  [path_rewrite] replaced {old!r} â†’ {migrated_base!r}")

    # 2. Fix every %run line.
    #
    # AIDP cannot resolve relative %run paths the way Databricks can â€” every
    ...
    def _fix_run(m):
        ...
        # arguments â€” an inline `# comment` is parsed as a path arg and
        ...
        # â€” Priority 1: dep_path_map lookup on raw (handles abs + relative via
        #   suffix/basename match in _lookup_dep_map) â€”
        dep_hit = _lookup_dep_map(raw)
        if dep_hit:
            return _emit(dep_hit, "path â†’ migrated dep path")

        # â€” Priority 2: Relative path resolution via nb_migrated_dir â€”
        if is_relative and nb_migrated_dir:
            ...
            if dep_hit2:
                return _emit(dep_hit2, "relative path â†’ migrated dep path")
            return _emit(fixed, "relative path â†’ full migrated path")

        # â€” Priority 3: Relative path with NO nb_migrated_dir â€” anchor at
        ...
        if is_relative and migrated_base:
            ...
            if not fixed.endswith(".ipynb") and not fixed.endswith(".py"):
                fixed += ".ipynb"
            return _emit(fixed, "relative path â†’ anchored at migrated_base")

        # â€” Priority 4: Absolute path â€” replace old staging bases + normalise â€”
        fixed = raw
        for old in _OLD_NB_BASES:
            if old in fixed:
        ...
        # â€” Priority 5: If still pointing at /Workspace/<user>/... (cross-user
        ...
        if (fixed.startswith("/Workspace/") or fixed.startswith("Workspace/")) \
            ...
            return _emit(fixed, "cross-user path â†’ migrated_base")
    ...
    #    â†’ oidlUtils.notebook.run(<absolute migrated path>, ...)
    #
    ...
    def _resolve_run_path(raw_path: str) -> str:
        ...
        # Priority 3: relative with no nb_migrated_dir â€” anchor at migrated_base
        if is_relative and migrated_base:
            stripped = raw_path
            while stripped.startswith("./") or stripped.startswith("../"):
                stripped = stripped[2:] if stripped.startswith("./") else stripped[3:]
        ...
        # Priority 4: absolute path â€” replace old staging bases + normalise
        fixed = raw_path
        for old in _OLD_NB_BASES:
            if old in fixed:
                fixed = fixed.replace(old, migrated_base)
        ...
        # Priority 5: cross-user absolute â†’ redirect under MIGRATED_BASE
        if (fixed.startswith("/Workspace/") or fixed.startswith("Workspace/")) \
                and migrated_base and not fixed.startswith(migrated_base):
            normalized = normalize_nb_path(fixed)
            if not normalized.endswith(".ipynb") and not normalized.endswith(".py"):
    ...
    def _fix_notebook_run(m):
        ...
        # The regex only matches up to the path quote â€” appending here lands
    ...
    # AIDP rejects notebook.run timeoutSeconds=0/null â€” Databricks used 0 for
    ...
    # Rename-only API rewrites (taskValues, notebook.exit). Idempotent â€” safe to
    # apply here even though parent cells also get these via CELL_MIGRATE_PROMPT.
    source = _rewrite_dbutils_api_renames(source)

    # Deterministic source-catalog â†’ default remap in string literals (e.g.
...
# â”€â”€ Auto dbutils Import Injection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# language: 'python' | 'scala' â€” scala entries only applied to %scala cells.
_IMPLICIT_IMPORT_RULES: list = [
    # â”€â”€ Scala: org.json (Databricks pre-imports, AIDP does not) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Scala: common Databricks-pre-available Spark/Java â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Python: common names used without import in Databricks notebooks â”€â”€â”€â”€â”€â”€
    (r'\bOrderedDict\b(?!\s*=)',
...
# â”€â”€ Databricks job-trigger rewriting â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
#
# AIDP `parameters` shape is list[{"key": str, "value": str}] â€” confirmed via
...
#
# Detection patterns (V1 â€” most common in source notebooks):
...
# print so it shows up in cell output. We do NOT raise â€” the cell continues â€”
# and the unmapped id is surfaced in JOB_REPORT for follow-up.
_AIDP_UNMAPPED_TEMPLATE = """\
{indent}# Oracle tool modification: UNMAPPED Databricks job {db_id} â€” add to manifest db_to_aidp_job_map and re-run
{indent}print("[Oracle migration] SKIPPED Databricks job {db_id}: no AIDP mapping in manifest db_to_aidp_job_map â€” see JOB_REPORT.md")"""

# â”€â”€ AIDP run_job_and_wait helper (inlined into every notebook that uses it) â”€â”€
...
# (mirrors /Workspace/dbc/invoke_job1.py â€” kept self-contained so the migrated
...
def _normalize_job_triggers(source: str) -> tuple:
    ...
    # 1) Build var â†’ int_job_id map from this cell's `job_id = <int>` assignments
    ...
    while i < len(lines):
        ...
        if job_arg.isdigit():
            ...
            out_lines.append(line)
            out_lines.append(f"{indent}# Oracle tool modification: job_call() with non-literal job_id {job_arg!r} â€” manual review required")
            i += 1
            changed = True
            continue
        ...
        if lhs_raw.endswith("="):
            ...
            if "," not in lhs_clean and lhs_clean.isidentifier():
                ...
                # AIDP returns a single status string â€” use _status and surface a note.
                lhs_for_template = "_status = "
                out_lines.append(
                    f"{indent}# Oracle tool modification: original was tuple unpack {lhs_raw}â€” AIDP returns single status string"
                )
...
# â”€â”€ Table reference normalization (Hive 1/2-part â†’ AIDP 3-part) â”€â”€â”€â”€â”€â”€
#
...
# or 2-part (`db.tbl`) â€” these silently fail on AIDP. We tag everything to
...
#   - SQL inside spark.sql("...") / spark.sql("""...""")  â€” single & multiline
#   - SQL bodies of %sql cells
#
# Skips:
#   - already 3-part references (cat.schema.tbl)
#   - CTE references (WITH cte AS ... â†’ cte is not a table)
...
def _to_three_part(ident: str) -> tuple:
    ...
    if len(parts) == 3:
        ...
        # or a source workload's 'analytics.gold.orders'). Now the operator opts in
...
# â”€â”€ Catalog-name remap in STRING LITERALS (source catalog â†’ default) â”€â”€
...
# (known source names only â†’ zero over-match; generic, not a hardcoded catalog).
...
def register_catalog_remap(manifest_path: str, default_catalog: str = "default") -> None:
    ...
    if remap:
        tprint(f"[catalog-remap] loaded {len(remap)} sourceâ†’{default_catalog} name remap(s)")
...
def _apply_catalog_remap(source: str) -> str:
    ...
    'main.users.bar' â€” that latter name needs its own manifest entry.
    """
...
_SQL_KEYWORDS_RE = re.compile(
    ...
    # DESCRIBE variants â€” list ALL the modifier keywords that can appear
    ...
    # ident=DETAIL, and DETAIL gets a `default.default.` prefix â€”
...
_SPARK_SQL_RES = [
    # Triple-quote forms â€” handle Python `r/f/u` and Scala `s/f` prefixes
...
def _normalize_table_refs(source: str) -> tuple:
    ...
    # 3. %sql cells â€” entire body after the magic line
...
def _preprocess_scala_cell_source(source: str, dep_path_map: dict = None) -> str:
    ...
    transforms in `_preprocess_cell_source` â€” they would corrupt the call.
    But path strings INSIDE notebook.run() calls still need to be normalized
    to match actual AIDP file names (space â†’ underscore, +.ipynb, dep_path_map
    ...
    def _lookup(raw):
        # Try multiple key variants â€” dep_path_map stores several forms per dep
        candidates = [raw]
    ...
    def _normalize_path(raw):
        ...
        if hit:
            return hit
        # No mapping â€” apply minimal AIDP-compatible normalization
        fixed = raw
        if "/Workspace/" in fixed:
            fixed = fixed.replace(" ", "_")
        if not fixed.endswith(".ipynb") and not fixed.endswith(".py"):
    ...
    # (dbutils|oidlUtils).notebook.run("path", ...) â€” also do API rename
    pattern = _re.compile(
    ...
    # â”€â”€ Table-ref normalization runs BEFORE the Cassandra block â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # substitutions emit later won't be re-qualified to 3-part â€” which would otherwise
    ...
    # â”€â”€ Cassandra/Scylla â†’ AIDP metastore (deterministic safety net) â”€â”€
    # Handles the most common Cassandra patterns before the cell is sent to OpenAI model, so
    # we get correct output even if the LLM drifts. Applied ONLY to %scala cells
    # that actually contain Cassandra references (no-op otherwise â€” cheap check).
    # This is a TEMPORARY mapping: keyspace â†’ scylla_<keyspace> schema, table name preserved.
    # Anything fancier than the patterns below falls through to OpenAI model + CELL_MIGRATE_PROMPT
    # rules (see SCYLLADB â†’ AIDP METASTORE MIGRATION section).
    ...
    if _has_cassandra:
        ...
        # 0a. Strip Cassandra/datastax imports â€” connector is not available on runtime cluster.
        _before = modified
        modified = _re.sub(
        ...
        # so `session` is no longer referenced â€” the wrapper is dead weight that ALSO requires
        ...
        def _strip_withsession_wrapper(text):
            ...
            while iter_count < MAX_ITER:
                ...
                while i < len(out) and depth > 0:
                    ...
                    i += 1
                if depth != 0:
                    # Unmatched brace â€” bail out without modifying
                    return out, out != text
                brace_close = i - 1
                ...
                # regex anchor (CassandraConnector...withSessionDo...{) â€” otherwise the next
        ...
        # `scylla_` for easy identification (e.g. sample_keyspace â†’
        # scylla_sample_keyspace). Original keyspace identifier is preserved
        # inside the prefix â€” Scala s-string interpolation handles the rest: a literal
        # "scylla_" + "$keyspaceVar" + ".<table>" â†’ at runtime "scylla_<actualKeyspace>.<table>".

        # 1. session.execute("DROP TABLE â€¦") â†’ spark.sql with IF EXISTS + scylla_ schema prefix
        _before = modified
        modified = _re.sub(
        ...
        if modified != _before:
            _scylla_changed = True
        # 2. session.execute("TRUNCATE â€¦") â†’ spark.sql("TRUNCATE TABLE â€¦") + scylla_ schema prefix
        _before = modified
        modified = _re.sub(
        ...
        if modified != _before:
            _scylla_changed = True
        # 3. system_schema.tables enumeration â†’ SHOW TABLES IN scylla_$ks (handles both
        ...
        if modified != _before:
            _scylla_changed = True
        # 4. Bulk DataFrame Cassandra read â†’ spark.read.table with scylla_ schema prefix
        _before = modified
        modified = _re.sub(
        ...
        if modified != _before:
            _scylla_changed = True
        # 5. Bulk DataFrame Cassandra write â†’ saveAsTable with overwrite + scylla_ schema prefix
        _before = modified
        modified = _re.sub(
            ...
            r' Temporary â€” restore original semantics when Scylla becomes directly reachable on AIDP.',
            modified,
        )
        if modified != _before:
            _scylla_changed = True
    ...
    # dbutils.widgets.get("X") â†’ oidlUtils.parameters.getParameter("X", "") (Scala).
    ...
    # already does this conversion, but Scala cells use THIS preprocessor â€” so without
    # it, the conversion was left to the (flaky) OpenAI model fix loop. Do it deterministically.
    # At EXEC time, _scala_param_exec_rewrite further bridges
    # oidlUtils.parameters.getParameter â†’ spark.conf.get("spark.aidp.param.X"), so the
    ...
    if _WIDGETS_GET_RE.search(modified):
        ...
        for _line in modified.split("\n"):
            ...
            if (not _ls.startswith("//")) and (not _ls.startswith("#")) and _WIDGETS_GET_RE.search(_line):
                ...
                _wlines.append(
                    f"{_pad}// Oracle tool modification: dbutils.widgets.get â†’ oidlUtils.parameters.getParameter (AIDP Scala)\n"
                    f"{_pad}// {_ls}\n"
                    f"{_fixed}")
            else:
...
def _preprocess_cell_source(source: str, dep_path_map: dict = None) -> str:
    """Programmatic pre-processing applied to every cell source BEFORE OpenAI model migration.

    Handles transformations that OpenAI model cannot reliably do on its own:
    1. pandas S3 reads â†’ wrap path with translate_path() (ocifs handles oci://)
    2. Old /Workspace/ internal paths â†’ migrated paths from dep_path_map
    """
    if not source or not source.strip():
        return source
    ...
    # â”€â”€ 1. pandas S3 reads â†’ wrap path with translate_path() â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if translate_import_needed and "translate_path" not in modified:
        modified = "from aidp_compat import translate_path  # Oracle tool modification: added for S3â†’OCI path translation\n" + modified

    # â”€â”€ 2. %matplotlib inline â†’ comment out â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # AIDP is a headless environment â€” %matplotlib inline is a Jupyter/Databricks magic
    ...
    # â”€â”€ 2-restart. dbutils.library.restartPython() â†’ comment out â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ 11a. Unicode normalization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Notebooks copy-pasted from docs/Slack often contain invisible Unicode chars
    # that break Python parsing in subtle ways:
    #   - U+00A0 (non-breaking space)  â†’ looks like space, parser rejects
    #   - U+201C/U+201D (curly quotes) â†’ not valid Python string delimiters
    #   - U+2018/U+2019 (curly apos)   â†’ not valid Python string delimiters
    #   - U+2013/U+2014 (en/em dash)   â†’ confusing in arithmetic / option flags
    #   - U+200B (zero-width space)    â†’ invisible breakage
    _UNICODE_FIXES = [
        ("Â ", " "),    # non-breaking space â†’ regular space
        ...
        ("â€“", "-"),    # en dash
        ("â€”", "-"),    # em dash
        ("â€‹", ""),     # zero-width space (just remove)
        ("ï»¿", ""),     # BOM (just remove)
    ]
    ...
    # â”€â”€ 11b. Triple-quote / unbalanced-quote table reads â”€â”€â”€â”€ REMOVED â”€â”€
    # Previously this transform tried to fix mismatched triple-quote typos
    # like `spark.read.table("""X"")` â†’ `spark.read.table("X")`. The regex
    # `(''')([^"'\n]+?)(''|')` looked for `'''<body><single-or-double-quote>`
    # with body excluding any quote.
    #
    # Why this was REMOVED â€” Bug B (root-cause):
    ...
    # OpenAI model's call_fix loop eventually recovered each occurrence, but at the
    # cost of extra API calls and risk of mis-fix. Removing the transform
    # entirely is safer than trying to make the regex precise â€” real
    # mismatched-triple-quote typos in source code are very rare, and
    # OpenAI model's normal migration + fix pipeline handles them when they occur.

    # â”€â”€ 11c. Strip inline comments from %run lines â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # IPython parses everything after `%run <path>` as args/params â€” `# foo`
    ...
    def _strip_run_comment(m):
        prefix = m.group(1)   # leading whitespace + '%run '
        rest   = m.group(2)
        # Cut at first ' #' (space-hash) â€” preserves $params with no '#' in them
    ...
    # â”€â”€ 2a. %%scala â†’ %scala normalization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ 2b. notebook.run(path, 0, ...) â†’ notebook.run(path, 3600, ...) â”€â”€
    # Databricks accepts timeout=0 as "no timeout"; AIDP rejects 0 as null/invalid.
    # Replace literal 0 timeout with 3600 (1 hour) â€” the second positional arg.
    ...
    # â”€â”€ 2c. OCI client init (oci.config.from_file + ObjectStorageClient) â†’ wrap in try/except â”€â”€
    ...
    # â”€â”€ 3. Old /Workspace/ internal paths â†’ migrated paths â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Uses dep_path_map (original_path â†’ migrated_path) built from the manifest.
    # Plain string replace (no inline comment) â€” the path may appear inside
    ...
    # â”€â”€ 4. get_glue_table_s3_location call-site rewrite REMOVED (2026-05-19) â”€
    ...
    #
    # â”€â”€ 5. spark.read.format('hudi').load(var) rewrite REMOVED (2026-05-19) â”€
    # Tied to rule 4 above â€” it converted Hudi path reads into
    # spark.read.table calls assuming <var> was a catalog identifier
    # set by rule 4. With rule 4 removed, <var> is still a path string,
    # so the Hudi â†’ spark.read.table conversion would now be wrong.
    # Hudi-specific migrations (if needed) are handled by OpenAI model per-cell
    # with full prompt context.

    # â”€â”€ 6. dbutils.widgets.get(...) â†’ oidlUtils.parameters.getParameter(..., "") â”€â”€
    ...
    if _WIDGETS_GET_RE.search(modified):
        ...
        for line in modified.split("\n"):
            ...
            if (not stripped.startswith("#")) and _WIDGETS_GET_RE.search(line):
                ...
                new_lines.append(
                    f"{pad}# Oracle tool modification: dbutils.widgets.get â†’ oidlUtils.parameters.getParameter (AIDP)\n"
                    f"{pad}# {stripped}\n"
                    f"{fixed_line}"
                )
            else:
    ...
    # â”€â”€ 7-job. Databricks job triggers â†’ AIDP run_job_and_wait â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ 7. Table refs â†’ 3-part `default.default.tbl` / `default.db.tbl` â”€â”€
...
def _ensure_dbutils_import(cells: list) -> list:
    ...
    Safe to call multiple times â€” skips if import already present.
    """
    uses_dbutils = any(
    ...
    # All code cells are magic cells â€” insert a dedicated import cell at the top.
    import_cell = {
        "cell_type": "code",
        "metadata": {},
...
def _ensure_invoke_job_helper(cells: list) -> list:
    ...
    Idempotent â€” skips if the helper definition is already present.
    """
    uses_helper = any(
...
# â”€â”€â”€ Inline Child Notebook Execution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def _inline_child_notebook(
    ...
    # Strategy: in-memory cache â†’ cluster filesystem â†’ AIDP source API download.
    # The in-memory cache bypasses FUSE entirely â€” os.path.exists() on AIDP's
    ...
    # same kernel, same session, no reconnects â€” not a cache timeout issue).
    from context_tools import _unwrap_aidp_text
...
""", timeout=30)
    ...
    # FUSE's os.path.exists() is unreliable on AIDP â€” returns False even for files that
    ...
    if child_cells is None:
        ...
        # Try child_path itself first â€” it's usually MIGRATED_BASE/relative and the file
...
""", timeout=30)
    ...
    # --- Source 4: AIDP source API download (original â€” last resort) ---
    if child_cells is None:
    ...
    # into a Scala cell â†’ java.lang.RuntimeException). Cells that already carry a
    ...
    if child_lang in ("scala", "sql"):
        ...
        for _i, _c in enumerate(child_cells):
            if _c.strip() and not _c.lstrip().startswith("%"):
                ...
                _prefixed += 1
        log.log(f"    [child:{child_name}] native {child_lang} notebook â€” made {_prefixed} bare cell(s) explicit ({_nmagic})")
    ...
    for ci, child_source in enumerate(child_cells):
        ...
        # Auto-fix to `pass` immediately â€” saves 9+ OpenAI model rounds per cell.
        if any(m in stripped for m in _CORRUPTION_MARKERS):
            log.log(f"    [child:{child_name}] Cell {ci}/{len(child_cells)}: CORRUPTION DETECTED â€” replacing with pass")
            child_cells[ci] = "pass"
            child_source = "pass"
            stripped = "pass"
            cells_fixed += 1
        ...
        if stripped.startswith("%run"):
            ...
            # Check uncommented lines only â€” commented-out notebook.run() is dead code
        ...
        if run_path_to_inline:
            ...
            if not resolved:
                ...
                # resolved against child_dir), candidate may equal run_path_to_inline â€”
        ...
        # the kernel so their definitions land in scope for later cells â€” a %run'd
        ...
        # are execute-first, so without this they keep dbutils.jobs.taskValues â€”
        ...
        # Execute it directly. If it fails, THEN call OpenAI model to fix.
        ...
        for attempt in range(MAX_CHILD_RETRIES):
            ...
            # child reads the default ("") â€” dropping every inter-notebook arg
            # (e.g. smsDumpPath). Mirror the parent: bridge Scala getParameter â†’
            ...
            # path. Child cells may write to OCI/tables too â€” must be
            # redirected so source data is never touched. Reads use any
            ...
            if _CLUSTER_DOWN_RE.search(output or ""):
                log.log(f"    [child:{child_name}] Cell {ci}: AIDP compute cluster down â€” reconnecting WS...")
            ...
            if _child_nb_exit:
                ...
                log.log(f"    [child:{child_name}] Cell {ci}/{len(child_cells)}: notebook.exit() â€” stopping child: {exit_msg}")
                cells_ok += 1
            ...
            if status != "ok":
                ...
                # Failing command: ..." â€” catch those explicitly.
            ...
            if attempt < MAX_CHILD_RETRIES - 1:
                ...
                try:
                    ...
                    # Include fix history so OpenAI model doesn't revert previous fixes
                    fix_history = ""
                    if fix_log:
                    ...
                    # OCI namespace is authoritative from the source/mapping â€” OpenAI model
        ...
        if cell_ok:
            ...
            log.log(f"    [child:{child_name}] Cell {ci}/{len(child_cells)}: FAILED â€” keeping code UNCHANGED (not commented) and CONTINUING to remaining cells")
            migrated_child_cells.append(child_source)
            child_cell_outputs.append([])
    ...
    # Safety: never write back to original workspace notebooks â€” only to migrated copies
    if not child_path.startswith(MIGRATED_BASE):
    ...
    # backslashes, newlines) â€” child_path can now come from manifest-supplied
...
""", timeout=30)
    ...
    if on_disk_count >= 0 and on_disk_count != len(migrated_child_cells):
        log.log(
            ...
            f"but migrated_child_cells has {len(migrated_child_cells)} â€” drift detected, "
    ...
    # Rewrite ALL code cells in the notebook â€” not just diffs.
...
""", timeout=60)
    ...
    if child_nb_exit:
        # Extract exit value from the last output (notebook.exit("value") â†’ "value")
    ...
    # source wrapper functions (createTable, drop_table, etc.) in the
    # kernel namespace at runtime. That approach was REPLACED with the
    # cell-text AST rewrite in _apply_wrapper_call_redirect() â€” the
    ...
    # OpenAI model inline-defined the wrapper functions without our wrapping).
...
# â”€â”€â”€ Upload Verification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
""", timeout=15)
            ...
            if "MISSING" not in out:
                ...
                # than int() the whole thing â€” guards against any residual AIDP
...
# â”€â”€â”€ Notebook Processing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def process_notebook(
    ...
    finalizing the PASS verdict. See $HOME/.aidp-migrator/engine/scripts/acceptance_contract.py.
    """
    ...
    try:
        ...
        # â”€â”€ Detect native Scala/SQL/R notebooks â€” AIDP supports Scala & SQL natively, no conversion needed â”€â”€
        _first_code = ""
        for _c in cells:
        ...
        if is_native_scala or is_native_sql:
            ...
            if is_native_scala:
                log.log(f"Native Scala notebook detected â€” cells migrated AS Scala via OpenAI model (never ported to Python)")
            else:
                log.log(f"Native SQL notebook detected â€” cells preserved as-is (no OpenAI model migration)")
        ...
        # â”€â”€ Step 0: Gather context for OpenAI model â”€â”€
        ...
        # returns "" for a bare filename â€” in that case the fix simply doesn't
        ...
        for dep in deps:
            ...
            # as absolute â€” matches the original code's behavior.
        ...
        # â”€â”€ Step 1: Cell-by-cell analysis (static + cluster verification) â”€â”€
        ...
        if DIRECT_EXECUTE:
            # Skip AI analysis â€” execute cells as-is (useful for WS resilience testing)
            cell_plans = [
        ...
        # â”€â”€ Step 1b: Install missing Python packages upfront â”€â”€
        ...
        # â”€â”€ Step 2: Inject parameters + bootstrap (only if executing) â”€â”€
        if run_all:
            if parameters:
            ...
            # `import system_config` / `from modules.config import â€¦` resolve
            # without OpenAI model having to add them manually each retry.
            local_syspath_block = ""
            if local_module_src_roots:
                local_syspath_block = (
                    "import sys as _sys\n"
            ...
            bootstrap = (
                ...
                # Object Storage CircuitBreaker + retry hardening â€” runtime-only.
            ...
            # â”€â”€ Write per-task manifest_params.json on the cluster â”€â”€â”€â”€â”€
...
# Verification probe â€” confirm the wrapper picks up the new params
...
if _probe_key:
    ...
    if _probe == _expected:
        ...
        print(f'[AIDP-BRIDGE] FAIL (wrapper={{_wrapper_type}}, probe sentinel returned â€” bootstrap snippet did not install wrapper)')
    else:
...
"""
            ...
            # Runtime-only toPandas() safety â€” prevents driver OOM during execution.
            # NOT saved in the migrated notebook.
            await session.execute(AIDP_TOPANDAS_SAFETY, timeout=30)
            # Brief pause â€” AIDP kernel can stall on rapid back-to-back requests
            await asyncio.sleep(2)

        # â”€â”€ Step 3: Cell-by-cell migrate (+ execute + fix if run_all) â”€â”€
        ...
        #   cells_data_substituted â€” passed via OK_DATA_SUBSTITUTED path
        #   cells_data_unavailable â€” failed after recovery exhausted dates
        #   cells_not_validated    â€” migrated code-only after task_validation_stop
        #   task_validation_stop   â€” once True, remaining cells skip execution
        ...
        # If yes, all subsequent cells are unreachable â€” copy them as-is, skip analysis/migration/execution.
        ...
        for i, cell in enumerate(cells):
            ...
            # â”€â”€ Skip cells after an unconditional notebook.exit() â”€â”€
            ...
            if _seen_unconditional_exit or _plan_for_cell.get("action") == "skip_after_exit":
                ...
                cell_results.append({"cell": i, "status": "skipped", "reason": "unreachable â€” after unconditional notebook.exit()"})
                log.log(f"Cell {i}/{total_cells}: SKIPPED (unreachable after unconditional notebook.exit)")
                continue

            # Programmatic pre-processing before OpenAI model (pandas S3 reads, internal path updates).
            ...
            # â”€â”€ Detect unconditional exit in THIS cell (set flag for next iterations) â”€â”€
            # Unconditional = top-level (zero indentation) notebook.exit() â€” not inside if/for/try.
            ...
            # â”€â”€ Gather cell-level context (plan + accumulated context) â”€â”€
            ...
            # Local-module mirror context â€” included in every cell migrate / fix
            # so OpenAI model only writes to the mirror copy, never the source.
            mirror_ctx = ""
            if local_mirror_root_dst:
                mirror_ctx = (
                    "\n=== LOCAL MODULE SOURCE MIRROR ===\n"
                    f"Source tree (READ-ONLY, NEVER MODIFY): {local_mirror_root_orig}\n"
                    ...
                    f"       â†’  '{local_mirror_root_dst}/modules/utils/foo.py'\n"
            ...
            # â”€â”€ Check if this is a Slack/notification/external URL cell â€” preserve as Raw â”€â”€
            stripped_source = source.strip()
            _slack_markers = [
                "slack", "webhook", "send_slack", "slack_alert",
                "hooks.slack.com", "internal", "intranet", ".corp", "private",
                # Slack SDK API method/import markers â€” catches `client.chat_postMessage(...)`,
            ...
            if _is_notification:
                ...
                log.log(f"Cell {i}/{total_cells}: OK (Slack/notification â€” preserved as Raw)")
                continue

            # â”€â”€ Check if this is a %scala or %sql cell â€” keep as-is, AIDP supports both â”€â”€
            ...
            if is_sql:
                # â”€â”€ Native SQL: deterministic transforms only, execute as-is â”€â”€
                # SQL cells are overwhelmingly table reads (%sql SELECT ... FROM
                # table), which run on AIDP unchanged. No OpenAI model. Retry up to 2x
                ...
                for _sql_attempt in range(2):
                    try:
                        ...
                        # with ... Failing command: ..." â€” scan output too.
                        ...
                        if has_error:
                            if _sql_attempt < 1:
                                ...
                                await asyncio.sleep(2)
                                continue
                            log.log(f"Cell {i}/{total_cells}: FAIL (%sql â€” execution error)")
                            ...
                            log.log(f"Cell {i}/{total_cells}: OK (%sql â€” executed as-is{(' retry ' + str(_sql_attempt)) if _sql_attempt else ''})")
                        break
                    except Exception as e:
                        if _sql_attempt < 1:
                            ...
                            await asyncio.sleep(5)
                            continue
                        log.log(f"Cell {i}/{total_cells}: FAIL (%sql â€” {str(e)[:100]})")
                        cells_failed += 1
            ...
            if is_scala:
                # â”€â”€ Native Scala: EXECUTE-FIRST, OpenAI model (call_fix) only on error â”€â”€
                # Mirror the Python execute-verify-fix philosophy: apply the
                # deterministic transforms, run the cell AS-IS, and only hand it
                # to OpenAI model when it actually errors. Do NOT migrate working Scala
                # up-front â€” that over-migrates cells that already run on AIDP
                ...
                if not run_all:
                    # Code-only (dep) mode: deterministic transforms only â€” no
                    # execution and no OpenAI model. The dep is executed + fixed when it
                ...
                # â”€â”€ notebook.run INLINE (treat like %run) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                # For each (oidlUtils|dbutils).notebook.run(...) call, inline the
                # child the same way %run does: set the child's Map params in the
                # kernel, execute+OpenAI model-fix the child's cells AND save the fixed
                ...
                # Scan UNCOMMENTED source only â€” a commented-out notebook.run
                ...
                # Execute-first; OpenAI model (call_fix) ONLY on a real execution error.
                _SCALA_RETRIES = 5
                current_code = source
                _resolved = False
                for _scala_attempt in range(_SCALA_RETRIES):
                    ...
                    # cell) is left unchanged â€” correct for a real workflow.
                    exec_code = _scala_param_exec_rewrite(current_code)
                    # EXEC-ONLY: swap notebook.run(...) â†’ the inlined child's
                    ...
                    try:
                        ...
                        # Transient (WS drop / kernel restart): retry AS-IS â€” do
                        # NOT hand to OpenAI model (it would rewrite working code).
                    ...
                    # status still 'ok' and NO type=='error' output â€” e.g.
                    ...
                    if not has_error:
                        ...
                        if _scala_attempt > 0:
                            cells_fixed += 1
                            migration_notes.append(f"Cell {i}: Scala auto-fixed by OpenAI model (attempt {_scala_attempt})")
                            log.log(f"Cell {i}/{total_cells}: OK (%scala â€” fixed by OpenAI model, attempt {_scala_attempt})")
                        else:
                            log.log(f"Cell {i}/{total_cells}: OK (%scala â€” executed as-is)")
                        cell_results.append({"cell": i, "status": "ok", "code": current_code[:120], "note": "%scala executed" + (" (OpenAI model-fixed)" if _scala_attempt else "")})
                        _resolved = True
                        break
                    # Real code/data error â†’ hand to OpenAI model (Scala-aware), keep %scala.
                    if _scala_attempt < _SCALA_RETRIES - 1:
                        log.log(f"Cell {i}/{total_cells}: %scala error (attempt {_scala_attempt+1}/{_SCALA_RETRIES}) â†’ OpenAI model fix: {out[:200]}")
                        try:
                            current_code = await call_fix(
                            ...
                            if not current_code.lstrip().startswith("%scala"):
                                current_code = "%scala\n" + current_code
                            # OCI namespace is authoritative from the source/mapping â€”
                            # revert any OpenAI model-invented namespace change (e.g.
                        ...
                        log.log(f"Cell {i}/{total_cells}: FAIL (%scala â€” error after {_SCALA_RETRIES} attempts): {out[:200]}")
                        cells_failed += 1
                        cell_results.append({"cell": i, "status": "fail", "code": current_code[:120], "note": "%scala execution error after OpenAI model fixes"})
                        _resolved = True
                if not _resolved:
                    # All attempts hit transient session exceptions.
                    log.log(f"Cell {i}/{total_cells}: FAIL (%scala â€” unresolved after {_SCALA_RETRIES} transient attempts)")
                    cells_failed += 1
            ...
            # â”€â”€ Check if this is a %pip install cell â€” install via cluster libraries API â”€â”€
            ...
            if _pip_match and run_all:
                ...
                # Strip version pins (e.g. mlflow==2.14.2 â†’ mlflow)
                _pip_pkgs = [re.split(r'[>=<!\[]', p)[0] for p in _pip_pkgs if p]
                if _pip_pkgs:
                    log.log(f"Cell {i}/{total_cells}: %pip install â€” installing {', '.join(_pip_pkgs)} via cluster libraries API")
                ...
                cells_ok += 1
                cell_results.append({"cell": i, "status": "ok", "code": stripped_source[:120], "note": "%pip â†’ cluster libraries API"})
                continue

            # â”€â”€ Check if this is a %run or dbutils/oidlUtils.notebook.run cell â”€â”€
            ...
            if is_run_cell:
                ...
                # Ensure /Workspace prefix â€” cluster filesystem requires it
                ...
                if run_all:
                    ...
                    if parameters and run_all:
                        ...
                        # from manifest_params.json on every call â€” it survives %run
...
""", timeout=15)
                    ...
                    if child_result.get("status") == "ok":
                        ...
                        # %run shares parent namespace â€” notebook.exit() in child stops parent too
                        if "exit_value" in child_result:
                            log.log(f"Cell {i}/{total_cells}: child called notebook.exit() â€” stopping parent: {child_result['exit_value'][:100]}")
                            break
                    else:
                        cells_failed += 1
                ...
                if DIRECT_EXECUTE:
                    ...
                    # Native SQL: deterministic transforms only, NO OpenAI model. SQL
                    ...
                    if migrated_code != source:
                        migration_notes.append(f"Cell {i}: native SQL â€” deterministic path transforms (S3â†’OCI, %run); no OpenAI model")
                elif is_native_scala:
                    # Native Scala: migrate THROUGH OpenAI model, treated as Scala. The
                    # deterministic regex alone is not enough â€” it only matches
                    ...
                    # executeâ†’fix loop never fires. OpenAI model catches those cases at
                    # migrate time. OpenAI model MUST keep this as Scala and never port it
                    ...
                    _scala_directive = (
                        ...
                        "- NEVER rewrite or port this cell to Python â€” the output must be Scala.\n"
                    ...
                    # Deterministic post-OpenAI model transforms (safety net), same as the
                    ...
                    if migrated_code != source:
                        migration_notes.append(f"Cell {i}: native Scala â€” migrated by OpenAI model (Scala-preserving)")
                else:
                    # â”€â”€ First-try optimization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                    # Apply only the deterministic mechanical transforms (S3â†’OCI,
                    # %run path rewrite, idempotent prefix fix). If the resulting
                    # candidate contains no Databricks-specific markers, skip the
                    # upfront CELL_MIGRATE_PROMPT call entirely â€” the execute+
                    ...
                    if not _has_databricks_markers(_candidate):
                        # No Databricks markers â€” try as-is on the cluster.
                        ...
                        log.log(f"Cell {i}/{total_cells}: AS_IS_CANDIDATE â€” no DB markers, will try as-is")
                        if migrated_code != source:
                            migration_notes.append(f"Cell {i}: deterministic transforms only (no OpenAI model)")
                    else:
                        # â”€â”€ Migrate this cell with OpenAI model (with tool use for path exploration) â”€â”€
                        migrate_resp, usage = await call_opus_with_tools(CELL_MIGRATE_PROMPT,
                        ...
                        # Deterministic post-OpenAI model transforms (S3â†’OCI, path fix, missing imports)
                        migrated_code = _apply_s3_translations(migrated_code)
                        migrated_code = _rewrite_internal_paths(migrated_code, MIGRATED_BASE, notebook_path, dep_path_map)
                        migrated_code = _fix_path_replace_idempotency(migrated_code)
                        ...
                        if migrated_code != source:
                            migration_notes.append(f"Cell {i}: migrated by OpenAI model")

            # â”€â”€ Rewrite notebook.run() paths to migrated locations â”€â”€
            ...
            # â”€â”€ Code-only mode (deps): skip execution, just save migrated code â”€â”€
            if not run_all:
                ...
                if _unconditional_exit:
                    log.log(f"Cell {i}/{total_cells}: unconditional notebook.exit() â€” skipping {total_cells - i - 1} remaining cell(s)")
                    for j in range(i + 1, len(cells)):
                        migrated_cells.append(cells[j])
                    break
                continue

            # â”€â”€ Task validation stop: code-only continuation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            # Set after a prior cell hit DATA_UNAVAILABLE_AT_MIGRATION. Subsequent
            # cells get their code migrated but NOT executed â€” saved notebook is
            # complete end-to-end; runtime validates execution.
            if task_validation_stop:
                log.log(f"Cell {i}/{total_cells}: MIGRATED_NOT_VALIDATED â€” code-only (task validation halted earlier)")
            ...
            # â”€â”€ Execute + Verify + Fix loop (up to 10 attempts) â”€â”€
            MAX_CELL_RETRIES = 5
            ...
            # failure had an empty-data signature â€” used at max-retries to
            ...
            for attempt in range(MAX_CELL_RETRIES):
                ...
                # to disk â€” only used for cluster execution.
                exec_code = (recovery_override + current_code) if recovery_override else current_code
                # â”€â”€ Write-redirect (tool-only): never touch source data â”€â”€
                ...
                # â”€â”€ Cluster-down detection: reconnect WS + re-execute â”€â”€
                ...
                if _CLUSTER_DOWN_RE.search(output or ""):
                    log.log(f"Cell {i}/{total_cells}: AIDP compute cluster down â€” reconnecting WS...")
                    await session.force_reconnect()
                    log.log(f"Cell {i}/{total_cells}: WS reconnected â€” replaying bootstrap...")
                    try:
                ...
                # â”€â”€ Check for notebook.exit() â€” treat as successful early stop â”€â”€
                ...
                if _nb_exit:
                    exit_msg = output.strip().split("\n")[-1][:200]
                    log.log(f"Cell {i}/{total_cells}: notebook.exit() called â€” stopping execution: {exit_msg}")
                    cells_ok += 1
                ...
                if status != "ok":
                    ...
                    # legitimate `raise ValueError(...)` line â€” this used to trip a
                    ...
                    # Check 3a: OCI permission / infrastructure errors in output (unfixable by OpenAI model)
                    _oci_fatal_patterns = [
                    ...
                    for _ofp in _oci_fatal_patterns:
                        if _ofp in (output or ""):
                            verification_issues.append(
                                f"OCI PERMISSION ERROR (unfixable): '{_ofp}' â€” "
                                f"cluster lacks access to the target bucket/path. "
                                f"Fix IAM/bucket policies, do NOT retry with OpenAI model.")
                            break
                    ...
                    # Check 4: Ask OpenAI model to evaluate the output (skipped in --direct-execute mode)
...
- oidlUtils is a NATIVE AIDP module, pre-loaded in every kernel. Do NOT flag oidlUtils.notebook.exit(),
  oidlUtils.notebook.run(), or oidlUtils.parameters.* as wrong â€” these are the correct AIDP equivalents
...
- "Compute cluster is not running" â€” transient infrastructure error, not a code bug
...
call get_cell_history to scan and fixup_cell to rewind â€” but verify idempotency first (replay re-executes
...
WHEN TO REWIND: If this is attempt 7+ and the root cause appears to be upstream, prefer fixup_cell."""
                        ...
                        if eval_line.upper().startswith("ISSUE"):
                            verification_issues.append(f"OpenAI model evaluation: {eval_line}")
                ...
                # â”€â”€ If verified OK, accept the cell â”€â”€
                if not verification_issues:
                    ...
                    if recovery_override:
                        ...
                        # separately from cells_fixed â€” this isn't a code
                        ...
                        migration_notes.append(
                            f"Cell {i}: OK_DATA_SUBSTITUTED â€” code validated on cluster "
                            f"using dates substituted from upstream table; "
                            f"source workload's original date filter saved unchanged"
                        )
                    elif attempt > 0:
                        cells_fixed += 1
                ...
                # â”€â”€ Verification failed - fix with OpenAI model â”€â”€
                ...
                # this guarantees OpenAI model rewrites code that was correct. Sleep
                ...
                # Auto-install missing Python packages before handing to OpenAI model (max 3 per notebook)
                ...
                if _mnf_match and len(_auto_installed_modules) < _MAX_AUTO_INSTALLS:
                    ...
                    if _missing_mod not in _auto_installed_modules:
                        ...
                        try:
                            from cluster_lifecycle import install_missing_package
                            job_output_path = f"{OUTPUT_BASE}/{job_name}"
                            log.log(f"  [auto-install] Missing module '{_missing_mod}' â€” installing via cluster libraries API ({len(_auto_installed_modules)}/{_MAX_AUTO_INSTALLS})...")
                            _installed = await install_missing_package(
                                session.cluster_id, session, _missing_mod,
                                job_output_path, timeout=600,
                                notebook_paths=[notebook_path])
                            if _installed:
                                log.log(f"  [auto-install] {_missing_mod} installed â€” retrying cell")
                    ...
                    _missing_mod = _mnf_match.group(1)
                    log.log(f"  [auto-install] Skipped '{_missing_mod}' â€” reached max auto-installs ({_MAX_AUTO_INSTALLS}). Add to cluster libraries manually.")

                consecutive_failures += 1
                issue_summary = "; ".join(verification_issues)
                # Classify the FIRST failure (attempt 0) as empty-data or
                # not. This tag drives the terminal-failure branch below â€”
                # empty-data â†’ DATA_UNAVAILABLE_AT_MIGRATION (continue with
                # code-only), code error â†’ FAILED (existing terminal-stop).
                ...
                if DIRECT_EXECUTE:
                    # No AI fixing in direct-execute mode â€” just record the failure and move on
                    break

                # Early exit: OCI permission errors are unfixable by OpenAI model â€” skip retries immediately
                _has_oci_perm_error = any("OCI PERMISSION ERROR" in v for v in verification_issues)
                if _has_oci_perm_error:
                    log.log(f"Cell {i}/{total_cells}: OCI permission error â€” cannot be fixed by migration tool. Check cluster IAM/bucket policies.")
                    cells_failed += 1
                ...
                # Early exit: if OpenAI model confirmed table/data is missing via make_note,
                # retrying won't help â€” the table won't appear between attempts.
                ...
                if _has_missing_data_note and attempt >= 1:
                    log.log(f"Cell {i}/{total_cells}: table/data confirmed missing â€” skipping retries")
                    cells_failed += 1
                    cell_results.append({"cell": i, "status": "error", "code": current_code[:1000],
                ...
                # â”€â”€ Data-recovery attempt (ONE-SHOT, READ-ONLY cells only) â”€
                # If this is an empty-data failure and we haven't tried
                # recovery yet, ask OpenAI model to identify the date variable and
                ...
                # byte-identical to the source workload's original.
                # READ_ONLY restriction: WRITE cells (saveAsTable, .write.*,
                # etc.) are NEVER given substituted dates â€” a write with
                ...
                if (not recovery_attempted
                    ...
                    log.log(f"Cell {i}/{total_cells}: empty-data failure detected â€” attempting data recovery")
                    try:
                    ...
                    if _override:
                        ...
                        # Re-enter the loop â€” exec_code will be recomputed
                        ...
                        # has a code-side issue that OpenAI model can fix.
                ...
                if attempt < MAX_CELL_RETRIES - 1:
                    try:
                        ...
                        # Local-module mirror â€” OpenAI model must patch the MIRROR, not the source
                        if local_mirror_root_dst:
                            fix_context += (
                                "\n\n=== LOCAL MODULE SOURCE MIRROR ===\n"
                                f"Source tree (READ-ONLY, NEVER MODIFY): {local_mirror_root_orig}\n"
                        ...
                        # OCI namespace authoritative from source/mapping â€” never let
                        # OpenAI model change it (e.g. <DATALAKE_NAMESPACE> -> <WORKSPACE_NAMESPACE>).
                        current_code = _apply_namespace_from_mapping(current_code)
                        total_tokens += 500
                    ...
                    if original_failure_was_empty_data:
                        ...
                        log.log(f"Cell {i}/{total_cells}: DATA_UNAVAILABLE_AT_MIGRATION after {MAX_CELL_RETRIES} attempts â€” switching task to code-only continuation for remaining cells")
                        cell_results.append({
                        ...
                        migration_notes.append(
                            f"Cell {i}: DATA_UNAVAILABLE_AT_MIGRATION â€” upstream data not available for "
            ...
            if job_should_stop:
                ...
                # deterministic code-only migration (S3â†’OCI paths, workspace path
                ...
                # Databricks / half AIDP. OpenAI model is NOT called on these cells â€”
                ...
                log.log(f"JOB FAILED: stopping execution, code-only migrating {remaining} remaining cells (no OpenAI model, no execution)")
                for j in range(i + 1, total_cells):
                    _orig_cell = cells[j]
                    ...
                    if _orig_cell.get("cell_type") == "code":
                        ...
                        if _src.strip():
                            try:
                                ...
                                # On any transform error, keep the original cell as-is â€”
                                # safer than emitting partially-mangled code.
                                log.log(f"  [post-fail] Cell {j}: transform error: {str(_e)[:200]} â€” preserving as-is")
        ...
        # Task outcome â€” precedence: FAIL > PARTIAL > PARTIAL_DATA_UNAVAILABLE
        ...
        if len(migrated_cells) < total_cells:
            log.log(f"WARNING: migrated notebook has {len(migrated_cells)} cells, original has {total_cells} â€” possible cell loss")

        # â”€â”€ Optional acceptance contract (post-cell-pass drain check) â”€â”€
        ...
        if acceptance_contract_dict and run_all and overall == "PASS":
            try:
                ...
                log.log(f"[acceptance] contract parse error: {_pe} â€” skipping (treating as PASS)")
                contract = None
            except ImportError as _ie:
                log.log(f"[acceptance] module not available ({_ie}) â€” skipping")
                contract = None
...
"""
        ...
        # â”€â”€ Write-redirect summary (tool-only writes that never touched production) â”€â”€
        ...
        if _wr["tables"] or _wr["paths"]:
            ...
            test_report += (
                ...
                "redirected to a tool-owned destination so production data was never touched. "
                "The saved notebook KEEPS the source workload's original paths/tables â€” these redirects "
            ...
            if _wr["paths"]:
                ...
                test_report += "\n"
            if _wr["collisions"]:
                test_report += "**Redirect collisions â€” review:**\n\n"
        ...
        try:
            ...
            # notebooks â€” we don't add any spark.conf configuration cell.)
            # Strip cell outputs before saving â€” execution outputs (e.g. Optuna trial
            ...
            # the operator will produce their own outputs when they run it.
            stripped_cells = []
            for cell in migrated_cells:
                cell = dict(cell)
                cell["outputs"] = []
                cell["execution_count"] = None
                # Strip internal fix-log comments â€” these are migration debug
                ...
                # here so the operator never sees test-only substitutions).
                if cell.get("cell_type") == "code":
                    src = "".join(cell.get("source", []))
                    src = src.split("\n# === AIDP MIGRATION FIX LOG ===")[0].rstrip()
            ...
            # Artifact-time sys.path cell â€” only added when local modules were
            ...
            # output dir which the operator retains.
            prepend_cells = []  # no AIDP perf-config cell prepended
            if local_module_src_roots:
                syspath_src = (
                    "# AIDP local-module sys.path â€” added automatically by migration tool.\n"
                    "# Mirrors the source workload's in-tree .py source so package-style imports\n"
...
""", timeout=60)
                    ...
                    if upload_attempt < UPLOAD_MAX_RETRIES - 1:
                        log.log(f"Upload failed (attempt {upload_attempt + 1}/{UPLOAD_MAX_RETRIES}): {str(upload_err)[:200]} â€” retrying in {delay}s...")
        ...
        if not upload_ok:
            log.log("Workspace upload failed â€” attempting OCI Object Storage backup...")
            try:
                ocs_files = [
                    (analysis_path, "analysis_report.md"),
                    (migration_report_path, "migration_report.md"),
...
# â”€â”€â”€ Dependency Scanner & Recursive Migration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _resolve_relative_dep(parent_path: str, naive_resolved: str) -> str:
    ...
    if not children:
        return naive_resolved
    # Exact match â€” naive resolution is correct, no override
    ...
    if len(matches) == 1:
        ...
        return matches[0]
    # 0 or 2+ matches â€” leave unchanged so existing failure paths trigger
    return naive_resolved

# â”€â”€ Databricks job_id â†’ AIDP job_key mapping (per-job) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# Bypasses FUSE entirely for _inline_child_notebook â€” files written to /Workspace via
...
# returned False at 12:53:52 â€” same kernel, same session, 24 seconds apart).
_notebook_content_cache: Dict[str, str] = {}

# â”€â”€â”€ Migration Registry (cross-job skip) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Per-job registry file at {OUTPUT_BASE}/{job_name}/migration_registry.json.
# Each job writes only to its own file â€” no cross-job write conflicts.
...
try:
    ...
    # Corrupt/partial JSON (e.g. interrupted save). DON'T return {{}} silently â€”
...
async def registry_update(session: "AIDPSession", original_path: str,
    ...
    re-expands them. Always writes â€” progress is recorded regardless of
    --skip-migrated."""
...
def scan_for_run_deps(notebook_content: bytes) -> List[Tuple[str, Dict[str, str]]]:
    ...
    for cell in nb.get("cells", []):
        ...
        # Filter out commented lines â€” commented-out %run / notebook.run() is dead
...
def normalize_nb_path(path: str) -> str:
    ...
    Strips /Workspace/ prefix only â€” preserves the rest of the path as-is
    (Users/..., Repos/..., Git/..., or whatever subtree the workload uses).
    ...
    # tree mirrors the original user tree (Users/...) â€” and so a notebook
...
def registry_key(path: str) -> str:
    ...
    Handles inputs in various shapes:
      - "/Workspace/Users/<email>/foo.ipynb"  â†’ kept as-is
      - "Users/<email>/foo.ipynb"             â†’ "/Workspace/Users/<email>/foo.ipynb"
      - "/Users/<email>/foo.ipynb"            â†’ "/Workspace/Users/<email>/foo.ipynb"
      - JOB_ROOT-relative (e.g. ".bundle/x.ipynb") â†’ re-prepended with
        "/Workspace/" + JOB_ROOT if JOB_ROOT is set; bare "/Workspace/x" otherwise.

    Spaces are normalised to underscores (AIDP convention â€” paths on the
    ...
    p = p.lstrip("/")
    # Workspace-relative path (e.g. "Users/alice@x.com/foo.ipynb") â€” just
    # add the /Workspace/ prefix
    if p.startswith("Users/"):
        return "/Workspace/" + p
    # Otherwise it might be JOB_ROOT-relative â€” re-attach JOB_ROOT if set
...
async def ensure_migrated(
    ...
    Flow:
    1. Check if already migrated (cache) â†’ return cached path
    ...
    if not download_path.endswith(".ipynb"):
        download_path += ".ipynb"
    # When migrating an exported job, ALWAYS source from under the export base â€”
    ...
    # Check migration registry (cross-job) â€” only when skipping already-migrated.
...
""", timeout=15)
        ...
        if "VALID" in check_output and SKIP_MIGRATED:
            ...
            try:
                ...
                tprint(f"{indent}[dep] WARNING: could not remove corrupt migration "
                       f"{check_path}: {_rm_e!r} â€” may re-loop until cleared manually")
    except Exception as _chk_e:
        # Narrowed from a bare `except: pass` â€” log instead of swallowing.
        tprint(f"{indent}[dep] WARNING: existence/corruption check failed for "
               f"{check_path}: {_chk_e!r}")
    # Lock released here - OpenAI model API calls below run WITHOUT holding the lock
    ...
    if not content:
        ...
        # Do NOT cache None â€” caching a failure here permanently blocks retries
    ...
    if len(unique_sub_deps) > 3:  # Parallel safe: callers use singleton, not session refs
        ...
        # then run OpenAI model migrations in parallel for the ones that need it.
        ...
        try:
            from context_tools import _unwrap_aidp_text
            # Use run_stateless (pool session) â€” avoids lock contention with health ping
    ...
    if result.get("status") in ("PASS", "PARTIAL"):
        ...
        # Don't cache None â€” backfill pass / sibling tasks should be allowed
...
async def backfill_missing_deps(
    ...
    skipped because of:
      ...
      - deps introduced only after a parent was rewritten by OpenAI model
    ...
    for iteration in range(max_iterations):
        ...
        try:
            ...
            tprint(f"[backfill] list failed: {e!r} â€” aborting backfill")
            return stats
        ...
        try:
            ...
            tprint(f"[backfill] scan failed: {e!r} â€” aborting backfill")
            return stats
        ...
        for parent_file, target in run_refs:
            ...
            if not target.startswith("/") and not target.startswith("Workspace/"):
                ...
                # fully rewritten â€” surface it but don't try to migrate (no source path).
                continue
        ...
        # Loop again â€” newly-migrated deps may themselves reference more deps
...
# â”€â”€â”€ Job Processing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def _process_job_inner(job: dict, session: AIDPSession) -> dict:
    ...
    # written tables/paths resolve to the redirected target â€” but a NEW
    ...
    # that task's cluster connect (see the connect block below) â€” not here.
    ...
    # Populate Databricksâ†’AIDP job_id map from manifest. Keys are stringified
    ...
    if _db_to_aidp_job_map:
        tprint(f"  [job_map] loaded {len(_db_to_aidp_job_map)} Databricksâ†’AIDP job mappings")
    ...
    for layer_idx, layer in enumerate(layers):
        ...
        for task_key in layer:
            ...
            # â”€â”€ Cluster switching: connect or switch if task needs a different cluster â”€â”€
            ...
            # cluster before any redirected write runs. Idempotent per cluster â€”
            ...
            # Check if already migrated â€” filesystem check then registry
...
""", timeout=15)
            ...
            # Use manifest run_deps to fill gaps â€” migrate any deps that scan_for_run_deps missed
            ...
            for mdep_path in manifest_deps:
                ...
                if not already_mapped:
                    tprint(f"  [{task_key}] Manifest dep not found by scan â€” migrating: {mdep_path}")
                    migrated = await ensure_migrated(
            ...
            if consecutive_nb_failures >= CONSECUTIVE_NB_FAIL_THRESHOLD:
                ...
                break  # stop current layer too â€” cascading failures

            if result.get("job_should_stop"):
                tprint(f"  [{task_key}] job_should_stop â€” cell exhausted retries, but migration continues for remaining tasks")
                # Don't block future layers â€” migration should attempt all notebooks.
                # Only circuit_breaker (consecutive FAIL notebooks) halts the job.

    # â”€â”€ Backfill pass: migrate any transitive %run targets that slipped through â”€â”€
    # Catches cross-user deps that were missed because of transient download
    # failures, manifest gaps, or post-OpenAI model-rewrite-introduced references.
...
""", timeout=60)
        ...
        tprint(f"  Job report workspace upload failed: {str(jr_err)[:200]} â€” trying OCI Object Storage...")
        try:
    ...
    return {
        ...
        # SKIP_MISSING_DEPS is a graceful skip, not an error â€” excluded from this count.
        "total_errored": sum(
            1 for r in _task_rs
...
async def main():
    ...
    parser.add_argument("--bucket-mapping", default=None,
                        help="(Deprecated/ignored) S3â†’OCI bucket mapping is now supplied via "
    ...
    parser.add_argument("--only-tasks", default="",
                        help="Comma-separated task names to run (substring match on task_key). "
                             ...
                             "Example: --only-tasks '02_BaseData_Task,05_Feature_Task'")
    ...
    _skip_grp.add_argument("--skip-migrated", action="store_true", dest="skip_migrated", default=True,
                           help="Skip notebooks already migrated (default: enabled). "
                                "Works across jobs â€” if a notebook was migrated by any job, it is skipped.")
    ...
    parser.add_argument("--direct-execute", action="store_true",
                        help="Skip AI analysis/migration and execute notebook cells as-is. "
                             "Useful for testing WebSocket resilience without OPENAI_API_KEY.")
    ...
    # Apply environment config â€” must happen before any code uses these globals
    ...
    # The cell-execution session (cluster_session â†’ AIDPSession) is created with
