...
| Input | Entrypoint |
|---|---|
| **Path-based**: a folder of `.ipynb` / `.py` notebooks the user wants migrated | `$HOME/.aidp-migrator/engine/scripts/build_dag.py` |
| **Workflow-based**: a Databricks Job ID whose tasks the user wants migrated, preserving the task DAG | `$HOME/.aidp-migrator/engine/scripts/build_dag_from_workflow.py` |
