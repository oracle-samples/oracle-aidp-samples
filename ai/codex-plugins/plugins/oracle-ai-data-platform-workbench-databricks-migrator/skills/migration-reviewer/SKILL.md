...
description: Review a migrated .ipynb for correctness — NOT just "did it run". Catches latent issues the cell-execute loop missed — wrong write-mode, lost rows, dropped columns, hardcoded paths, dead Databricks-isms, builtins shadowing, format downgrades. Outputs a structured Blockers / Important / Style / Good review report. Use after aidp-migrate-job completes, especially on the first few notebooks of a new workload before scaling out.
---

# `migration-reviewer` — side-by-side migrated vs source diff

Code-review specialist for Databricks → AIDP migrated notebooks. DO NOT modify the notebook. Read the migrated `.ipynb` + the corresponding cell of the SOURCE notebook side-by-side and call out drift.

## When to use

- After [`aidp-migrate-job`](../aidp-migrate-job/SKILL.md) completes — especially on the first few notebooks of a new workload before scaling out.
- When [`migration-status`](../migration-status/SKILL.md) says PASS but the user wants a second-opinion correctness check (PASS = "all cells ran clean", not "the logic is right").
- Before signing off a migration in a regulated environment.
...
- If a finding is on the boundary ("might be fine, might be wrong"), categorize as Important and explain BOTH ways. Don't pretend false certainty.
...
- A blocker requires installing a library on the cluster (e.g. Delta connector) — the user must act, this skill cannot.
