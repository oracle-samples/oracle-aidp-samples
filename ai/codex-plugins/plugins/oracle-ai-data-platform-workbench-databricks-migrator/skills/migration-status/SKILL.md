...
description: Parse and summarize the JOB_REPORT.md produced by a prior migration run. Surfaces cell pass/fail/fix counts per task, lists the failing cells, and recommends a next-step skill (fixup-cell, resume, or all-clear). Use after any aidp-migrate-job run, or when the user asks "how did the migration go", "what failed", "did it pass".
---
...
- After any [`aidp-migrate-job`](../aidp-migrate-job/SKILL.md) run.
...
If the user gave you a path or a job name, target that. If empty, locate the newest.
...
| PARTIAL with N failing cells | Route to [`aidp-fixup-cell`](../aidp-fixup-cell/SKILL.md) for each. |
| FAIL on dep migration | Route to manual fix → [`aidp-resume-migration`](../aidp-resume-migration/SKILL.md). |
| ACCEPTANCE_CONTRACT_VIOLATED | Investigate why the pending queue didn't drain. See [`aidp-acceptance-contract`](../aidp-acceptance-contract/SKILL.md). |
