...
description: First-run setup for the self-contained AIDP plugin. Verifies the oci CLI + a DEFAULT api_key profile, installs the bundled Python deps, discovers region + DataLake OCID + workspace, then smoke-tests both engines (a control-plane GET /catalogs via oci raw-request and a trivial SELECT 1 cell via $HOME/.aidp/aidp_sql.py). Use on first run, when the user asks to "set up / configure / install / bootstrap AIDP", or when control-plane or SQL calls fail with auth/config errors. No MCP install required.
---
...
- **Interactive Spark-SQL / notebook cells** → the bundled helper `python "$HOME/.aidp/aidp_sql.py"` (mints a UPST
...
- **No api_key? Use a session token everywhere:** `oci session authenticate --profile <P> --region <r>`, then
  control-plane via `--auth security_token --profile <P>`, and `$HOME/.aidp/aidp_sql.py --profile <P>` for cells
...
The only code in this plugin is `$HOME/.aidp/aidp_sql.py`; it needs `oci`, `requests`, `websocket-client`,
`cryptography`.

> **The helper lives at `$HOME/.aidp/`** — the bundle's `INSTALL.md` copies `aidp_sql.py`,
> `requirements.txt`, and `references/` there, so every skill calls it by absolute path:
> `python "$HOME/.aidp/aidp_sql.py"`. If you installed it elsewhere, `export AIDP_HOME=<dir>` and
> substitute `$AIDP_HOME` for `$HOME/.aidp` in the examples below.

```bash
python -m pip install -r "$HOME/.aidp/requirements.txt"
```
...
2. **Interactive SQL** (`$HOME/.aidp/aidp_sql.py`) — pick an ACTIVE cluster
...
## References
- [references/oci-raw-request.md]($HOME/.aidp/references/oci-raw-request.md) — base URL, auth ladder, invocation shapes
- [references/no-mcp-rest-map.md]($HOME/.aidp/references/no-mcp-rest-map.md) — verified control-plane endpoints per skill
- [$HOME/.aidp/aidp_sql.py]($HOME/.aidp/aidp_sql.py) · [$HOME/.aidp/requirements.txt]($HOME/.aidp/requirements.txt) — the SQL engine

<!-- Optional accelerator: if an `aidp`-style MCP server is configured in `~/.codex/config.toml`
([mcp_servers.aidp]), you MAY use its tools as a convenience — but it is NOT required and the bundle never
assumes it. Everything above works with only the oci CLI + a DEFAULT api_key profile. -->
