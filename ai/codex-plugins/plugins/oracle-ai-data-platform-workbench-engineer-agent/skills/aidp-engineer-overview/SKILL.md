...
- **Connecting to an external source** (Oracle ADB/ExaCS, Fusion, EPM, Essbase, Snowflake, S3, Kafka, …) —
  **including a request like "a notebook that connects to Fusion"** → use the **AIDP Spark Connectors**
  skills bundle (the Codex port of `oracle-ai-data-platform-workbench-spark-connectors`): use its
  `aidp-<source>` skill for the connection recipe — **do NOT hand-roll the connection.** Its
  `oracle_ai_data_platform_connectors` helper package installs once to `/Workspace/Shared` via that bundle's
  **`aidp-connectors-bootstrap`** skill. A single external source needs only that connector skill (then
  author/run via `aidp-notebooks`); `aidp-federate` composes several for cross-source joins.
...
- **Engine precedence — self-contained, no MCP/private-repo dependency.** For every CONTROL-PLANE op
  ...
  - **Interactive Spark-SQL / notebook cells** run via the bundled helper:
    `python "$HOME/.aidp/aidp_sql.py" --region <r> --datalake <ocid> --workspace <ws> --cluster <key>
...
## References
- [references/aidp-cli-map.md]($HOME/.aidp/references/aidp-cli-map.md) — skill → official `aidp` CLI command (preferred engine)
- [references/oci-raw-request.md]($HOME/.aidp/references/oci-raw-request.md) — control-plane REST fallback + auth
- [references/no-mcp-rest-map.md]($HOME/.aidp/references/no-mcp-rest-map.md) — endpoint map per skill
- [references/payloads.md]($HOME/.aidp/references/payloads.md) — `.aidp/payloads/` mutation-persistence convention
- `$HOME/.aidp/aidp_sql.py` — bundled interactive Spark-SQL helper
- [references/mcp-tool-map.md]($HOME/.aidp/references/mcp-tool-map.md) — optional MCP accelerator (not required)
