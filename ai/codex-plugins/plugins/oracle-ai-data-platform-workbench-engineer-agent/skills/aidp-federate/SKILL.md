...
is via the bundled `$HOME/.aidp/aidp_sql.py` helper (no MCP, no AIDP_SESSION required).
...
- **Also the entry point for a SINGLE external source** ("a notebook that connects to Fusion / EPM / ADB / …"):
  the connection recipe still comes from the **AIDP Spark Connectors** bundle's `aidp-<source>` skill
  (step 1) — this skill just adds the join. **Never hand-roll the source connection.** (Its
  `oracle_ai_data_platform_connectors` helper package installs once to `/Workspace/Shared` via that bundle's
  **`aidp-connectors-bootstrap`** skill.)
...
## References
- spark-connectors plugin (source reads) · [$HOME/.aidp/aidp_sql.py]($HOME/.aidp/aidp_sql.py) — bundled Spark-SQL executor
- [references/semantic-model.md]($HOME/.aidp/references/semantic-model.md) · [references/no-mcp-rest-map.md]($HOME/.aidp/references/no-mcp-rest-map.md) · pairs with `aidp-analyzing-data`
