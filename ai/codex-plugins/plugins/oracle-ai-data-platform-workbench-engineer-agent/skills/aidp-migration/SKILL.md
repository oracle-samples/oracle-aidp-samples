...
execution runs via the bundled `$HOME/.aidp/aidp_sql.py`. No MCP server or `ai-data-engineer-agent` repo
is required.
...
3. **Port notebooks**: recreate notebooks in the workspace (`aidp-notebooks` / `aidp-workspace-files`),
   ...
   other formats). Validate cells run with the bundled helper (`python "$HOME/.aidp/aidp_sql.py" … --code …`).
...
- **Control-plane** (workspaces, catalogs, tables, clusters, jobs, files) → `oci raw-request` against the
  AIDP REST API — see [references/oci-raw-request.md]($HOME/.aidp/references/oci-raw-request.md) and
  [references/no-mcp-rest-map.md]($HOME/.aidp/references/no-mcp-rest-map.md).
- **Interactive Spark-SQL / cell execution** (validate ported cells, compare counts/aggregates) →
  `python "$HOME/.aidp/aidp_sql.py" --region <r> --datalake <OCID> --workspace <ws> --cluster <key> --code <…>`.
...
- [references/oci-raw-request.md]($HOME/.aidp/references/oci-raw-request.md) · [references/no-mcp-rest-map.md]($HOME/.aidp/references/no-mcp-rest-map.md) · `$HOME/.aidp/aidp_sql.py`
