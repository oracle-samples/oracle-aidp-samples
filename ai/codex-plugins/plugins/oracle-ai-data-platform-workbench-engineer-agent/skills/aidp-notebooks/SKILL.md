...
> ADB/ExaCS, Snowflake, S3, Kafka, …)? **Do NOT hand-roll the connection here.** Use the **AIDP Spark
> Connectors** bundle's `aidp-<source>` skill for the connection recipe (e.g.
> `aidp-fusion-rest`/`aidp-fusion-bicc`, `aidp-oracle-db`, `aidp-snowflake`, `aidp-object-storage`). Its
> `oracle_ai_data_platform_connectors` helper package installs once to `/Workspace/Shared` via that bundle's
> **`aidp-connectors-bootstrap`** skill. This skill then just authors + runs the notebook; for multi-source
> joins see `aidp-federate`.
...
- **CELL EXECUTION** (run python/Spark, persist kernel state) → the bundled `$HOME/.aidp/aidp_sql.py`,
  ...
  cells (running a notebook end-to-end is job-based). Interactive Spark-SQL stays on `$HOME/.aidp/aidp_sql.py`.

> **Live-verified 2026-06-10 on de-agent:** the full WebSocket path via `$HOME/.aidp/aidp_sql.py` — auto-create
...
[references/aidp-cli-map.md]($HOME/.aidp/references/aidp-cli-map.md)).
...
the op. Base (see [references/oci-raw-request.md]($HOME/.aidp/references/oci-raw-request.md) for host/version/auth):
...
in [references/oci-raw-request.md]($HOME/.aidp/references/oci-raw-request.md) (`--profile DEFAULT` api_key first).

For mutating ops (create/update/modify/delete content, patch/delete session), persist the request body to
`.aidp/payloads/` and confirm first — see [references/payloads.md]($HOME/.aidp/references/payloads.md).

## CELL EXECUTION (the core) — $HOME/.aidp/aidp_sql.py
...
1. Ensure the target cluster is RUNNING first (cluster start/status via the cluster skill / REST —
   see [references/no-mcp-rest-map.md]($HOME/.aidp/references/no-mcp-rest-map.md)).
...
- FILE ops follow the REST auth ladder (api_key `--profile DEFAULT`; on 401/403 fall back to
  `AIDP_SESSION` — see [references/oci-raw-request.md]($HOME/.aidp/references/oci-raw-request.md)).
...
These are **cell** constructs — run them through `$HOME/.aidp/aidp_sql.py --code "…"`, not the CLI.
...
Spark Structured Streaming runs as **cell** code via `$HOME/.aidp/aidp_sql.py --code "…"` (cluster RUNNING).
...
## References
- [references/aidp-cli-map.md]($HOME/.aidp/references/aidp-cli-map.md) — skill → official `aidp` CLI command map (primary engine)
- [references/oci-raw-request.md]($HOME/.aidp/references/oci-raw-request.md) — REST host/version/auth + invocation shapes (fallback)
- [references/no-mcp-rest-map.md]($HOME/.aidp/references/no-mcp-rest-map.md) — control-plane endpoints (clusters, jobs)
- [references/payloads.md]($HOME/.aidp/references/payloads.md) — persist + confirm request bodies for mutating ops
- `$HOME/.aidp/aidp_sql.py` — the bundled cell/SQL executor
