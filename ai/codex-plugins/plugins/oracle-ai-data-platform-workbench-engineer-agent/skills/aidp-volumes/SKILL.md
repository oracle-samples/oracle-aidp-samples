...
## CLI (preferred)
Per [references/aidp-cli-map.md]($HOME/.aidp/references/aidp-cli-map.md): `volume list | get | create | list-files |
...
to `.aidp/payloads/` and confirm with the user before running (see [references/payloads.md]($HOME/.aidp/references/payloads.md)).

**Fallback (no CLI)** — same REST + auth via `oci raw-request` against
`…/20240831/dataLakes/<DATALAKE_OCID>/…` (auth ladder in [references/oci-raw-request.md]($HOME/.aidp/references/oci-raw-request.md)):
